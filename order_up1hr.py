...
def analyze_order_book(symbol, data):
    try:
        ...
        if long_signal:
            ...
            sl = close_price - (atr*.5)
            return (f"15min{symbol.upper()}: LONG Signal | Entry: {format_price(close_price)}, TP1: {format_price(tp1)}, TP2: {format_price(tp2)}\n"
                    f"SL: {format_price(sl)}\n"
                    f"Support (Nearest Bid): {format_price(nearest_bid[0])}")
        elif short_signal:
            tp = close_price - (atr * risk_reward_ratio)
            sl = close_price + atr
            return (f"15min{symbol.upper()}: SHORT Signal | Entry: {format_price(close_price)}, TP1: {format_price(tp1)}, TP2: {format_price(tp2)}\n"
                    f"SL: {format_price(sl)}\n"
