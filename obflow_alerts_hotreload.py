...
def _maybe_alert(symbol: str, side: str, entry_price: float, tp_pct: float, sl_pct: float) -> None:
    ...
    msg = f"{symbol} | {side.upper()} | entry={entry_price:.6f} | TP={tp_price:.6f} | SL={sl_price:.6f} | {now_pt}"
    print("[ALERT]", msg)
    _send_telegram(msg)
    ...
    for sym in TRACK_SYMBOLS:
        try:
            ...
            if candidates:
                ...
                pick = candidates[0]
                _maybe_alert(sym, pick["side"], price, pick["tp_pct"], pick["sl_pct"])
