...
def analyze_order_book(symbol, data):
    try:
        ...
        if long_signal:
            ...
            return (f"fib_24hr{symbol.upper()}: LONG Signal | Entry: {format_price(close_price)}, TP: {format_price(tp)}, SL: {format_price(sl)}\n"
            ...
            return (f"fib_24hr{symbol.upper()}: SHORT Signal | Entry: {format_price(close_price)}, TP: {format_price(tp)}, SL: {format_price(sl)}\n"
