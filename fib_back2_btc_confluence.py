...
Plus: BTC Stoch RSI confluence (optional, logged; can be enforced via flag)

Entry (reversal):
  - Ascend into zone => SHORT
  - Descend into zone => LONG
  - REQUIRED filter: Stoch RSI strict (40/60), per-coin
  - OPTIONAL filter: BTC Stoch RSI alignment (logged; toggle enforcement via flag)
...
# ============ USER CONFIG ============
COINS = [
    "ETH-USD","XRP-USD","SOL-USD","ADA-USD","AVAX-USD","DOGE-USD","DOT-USD",
...
# BTC confluence configuration (NEW)
REQUIRE_BTC_CONFLUENCE = False  # keep False to only log; set True to enforce
...
def summarize_and_save(trades: list):
    ...
    if "btc_stoch_align" in df.columns:
        ...
        summary = pd.concat([overall, by_zone, by_btc], ignore_index=True)
    else:
