...
# -*- coding: utf-8 -*-
"""
obflow_alerts_rules_hotreload_earlycut_tracking.py
--------------------------------------------------
Per-symbol tightened-rule alerts (hot-reload JSON) + Telegram + Early Cut (all symbols) + Trade tracking.

- Gates from JSON (Windows path preferred), hot-reload
- Alerts include p_win and EV, prices dynamically formatted
- Early Cut:
    * SHORTS: check at +15m; cut if still adverse by buffer
    * LONGS:  check at +30m; cut if still adverse by buffer
    * Per-symbol/side override buffers supported
- Trade tracking:
    * Logs open trade on alert
    * Monitors TP/SL or EARLY_CUT; closes and logs outcome with realized R and duration
"""
...
SETUPS_REFRESH_SEC = int(os.getenv("SETUPS_REFRESH_SEC", "300"))  # 5 minutes

# -------------- Early Cut configuration --------------
EARLY_CUT_ENABLED = True
EARLY_CUT_SHORT_SEC = int(os.getenv("EARLY_CUT_SHORT_SEC", str(15*60)))  # 15 minutes for SHORTS
EARLY_CUT_LONG_SEC  = int(os.getenv("EARLY_CUT_LONG_SEC",  str(30*60)))  # 30 minutes for LONGS
EARLY_CUT_SHORT_BUFFER_PCT = float(os.getenv("EARLY_CUT_SHORT_BUFFER_PCT", "0.10"))  # 0.10%
EARLY_CUT_LONG_BUFFER_PCT  = float(os.getenv("EARLY_CUT_LONG_BUFFER_PCT",  "0.10"))  # 0.10%

# Optional: per-symbol/side override buffers (percent). Example entries included.
EARLY_CUT_OVERRIDES = {
    # "SYMBOL": {"long": 0.12, "short": 0.10}
    "ARB/USD": {"short": 0.10},
    "SOL/USD": {"long":  0.12},
    "XLM/USD": {"short": 0.10},
    "INJ/USD": {"short": 0.10},
...
_OPEN_TRADES: Dict[str, Dict[str, Any]] = {}      # trade_id -> trade dict
_LAST_TRADE_ID: Dict[Tuple[str,str], str] = {}    # (symbol, side) -> last trade_id
...
def _ensure_trade_csv():
    if not os.path.exists(TRADE_LOG_CSV):
        with open(TRADE_LOG_CSV, "w", newline="", encoding="utf-8-sig") as f:
            w = csv.writer(f)
            w.writerow([
                "trade_id","symbol","side","entry_time","entry_price","tp_pct","sl_pct","tp_price","sl_price",
                "p_win_est","ev_est","status","exit_time","exit_reason","exit_price","pnl_r","duration_min"
            ])
...
def _append_trade_row(row: Dict[str, Any]) -> None:
    ...
    with open(TRADE_LOG_CSV, "a", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow([
            row.get("trade_id"), row.get("symbol"), row.get("side"), row.get("entry_time"), row.get("entry_price"),
            row.get("tp_pct"), row.get("sl_pct"), row.get("tp_price"), row.get("sl_price"),
            row.get("p_win_est"), row.get("ev_est"), row.get("status"), row.get("exit_time"),
            row.get("exit_reason"), row.get("exit_price"), row.get("pnl_r"), row.get("duration_min")
        ])

def _make_trade_id(symbol: str, side: str) -> str:
    ts = datetime.now().strftime("%Y%m%d%H%M%S")
    return f"{symbol.replace('/','-')}|{side.upper()}|{ts}|{uuid4().hex[:6]}"

def _register_trade(symbol: str, side: str, entry_price: float, tp_pct: float, sl_pct: float,
                    p_win: Optional[float], ev: Optional[float]) -> str:
    trade_id = _make_trade_id(symbol, side)
    if side == "long":
        tp_price = entry_price * (1.0 + tp_pct/100.0)
        sl_price = entry_price * (1.0 - sl_pct/100.0)
        denom = (entry - sl_price) if (entry := entry_price) else np.nan
        r = (tp_price - entry) / denom if denom else np.nan
    else:
        tp_price = entry_price * (1.0 - tp_pct/100.0)
        sl_price = entry_price * (1.0 + sl_pct/100.0)
        denom = (sl_price - entry_price)
        r = (entry_price - tp_price) / denom if denom else np.nan

    now_pt = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    trade = {
        "trade_id": trade_id, "symbol": symbol, "side": side,
        "entry_time": now_pt, "entry_price": float(entry_price),
        "tp_pct": float(tp_pct), "sl_pct": float(sl_pct),
        "tp_price": float(tp_price), "sl_price": float(sl_price),
        "p_win_est": None if p_win is None else float(p_win),
        "ev_est": None if ev is None else float(ev),
        "status": "open", "exit_time": "", "exit_reason": "", "exit_price": "", "pnl_r": "", "duration_min": ""
    }
    _OPEN_TRADES[trade_id] = trade
    _append_trade_row(trade)   # write open snapshot
    _LAST_TRADE_ID[(symbol, side)] = trade_id
    return trade_id

def _close_trade(trade_id: str, exit_reason: str, exit_price: float) -> None:
    tr = _OPEN_TRADES.get(trade_id)
    if not tr: return
    entry = tr["entry_price"]
    if tr["side"] == "long":
        denom = (entry - tr["sl_price"])
        pnl_r = (exit_price - entry) / denom if denom != 0 else np.nan
    else:
        denom = (tr["sl_price"] - entry)
        pnl_r = (entry - exit_price) / denom if denom != 0 else np.nan
    try:
        start_dt = datetime.strptime(tr["entry_time"], "%Y-%m-%d %H:%M:%S")
        mins = (datetime.now() - start_dt).total_seconds() / 60.0
    except Exception:
        mins = np.nan

    tr["status"] = "closed"
    tr["exit_time"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    tr["exit_reason"] = exit_reason
    tr["exit_price"] = float(exit_price)
    tr["pnl_r"] = None if pnl_r is np.nan else float(pnl_r)
    tr["duration_min"] = None if mins is np.nan else round(float(mins), 2)

    _append_trade_row(tr)
    _OPEN_TRADES.pop(trade_id, None)

def _monitor_tp_sl_and_close() -> None:
    if not _OPEN_TRADES: return
    for tid, tr in list(_OPEN_TRADES.items()):
        try:
            ...
            if side == "long":
                if cur >= tr["tp_price"]:
                    _close_trade(tid, "TP", cur)
                elif cur <= tr["sl_price"]:
                    _close_trade(tid, "SL", cur)
            else:
                if cur <= tr["tp_price"]:
                    _close_trade(tid, "TP", cur)
                elif cur >= tr["sl_price"]:
...
def _price_near_level(price: float, level: Optional[float], pct: float = NEAR_PCT) -> int:
...
def _fib_golden_zone(lo: float, hi: float, tol: float = NEAR_PCT) -> Tuple[float,float]:
    r618 = lo + 0.618 * (hi - lo); r660 = lo + 0.660 * (hi - lo)
...
def _price_near_level(price: float, level: float, pct: float = NEAR_PCT) -> int:
    if not (np.isfinite(price) and np.isfinite(level)): return 0
    return int(abs(price - level) / price <= pct)
...
def fetch_depth_top(pair: str) -> Tuple[Optional[float], Optional[float]]:
    ...
    try:
        ...
        bids = dp.get("bids", []); asks = dp.get("asks", [])
        if bids: bid_top = float(bids[0][0])
        if asks: ask_top = float(asks[0][0])
    except Exception:
        pass
...
def _resolve_setups_path() -> Optional[str]:
    for p in [os.getenv("SETUPS_JSON_PATH") or "", SETUPS_JSON_PATH, SETUPS_FALLBACK]:
...
def _load_symbol_setups() -> None:
    ...
    if not path:
        print("[setups] no setups JSON found"); _SYMBOL_SETUPS = {}; return
    try:
        ...
        print("[setups] load failed:", e); _SYMBOL_SETUPS = {}
...
def _eval_rule_text(rule_text: str, feature_lookup: Dict[str, float]) -> bool:
    if not rule_text or rule_text.strip().lower().startswith("no strong"): return False
    parts = [c.strip() for c in rule_text.split("AND") if c.strip()]
    if not parts: return False
    for cond in parts:
        m = _COND_RE.match(cond)
        if not m: return False
        feat, op, val = m.groups(); feat = feat.strip()
        if feat not in feature_lookup: return False
        try:
            x = float(feature_lookup.get(feat)); thr = float(val)
        except Exception:
            return False
...
# -------------- Alert plumbing + formatting --------------
def _price_decimals(price: float) -> int:
    if price is None or not isinstance(price, (int, float)): return 6
    p = float(price)
    if p >= 1000: return 2
    if p >= 100:  return 3
    if p >= 1:    return 4
...
def _fmt_price(x: float) -> str:
    return f"{x:.{_price_decimals(x)}f}"
...
def _cooldown_ok(symbol: str, side: str) -> bool:
    ...
    if now - last >= ALERT_COOLDOWN_MIN * 60: _LAST_ALERT_TS[key] = now; return True
    return False
...
# -------------- Early Cut storage & helpers --------------
_PENDING_EARLY_CUTS: List[Dict[str, Any]] = []

def _compute_return_pct(side: str, entry: float, current: float) -> float:
    if side == "long":  return (current - entry) / entry * 100.0
    else:               return (entry - current) / entry * 100.0

def _get_buffer_pct(symbol: str, side: str) -> float:
    # Use per-symbol override if present; else default per side
    ov = EARLY_CUT_OVERRIDES.get(symbol, {})
    if side == "long":
        return float(ov.get("long", EARLY_CUT_LONG_BUFFER_PCT))
    else:
        return float(ov.get("short", EARLY_CUT_SHORT_BUFFER_PCT))

def _schedule_early_cut(symbol: str, side: str, entry_price: float) -> None:
    if not EARLY_CUT_ENABLED: return
    if not isinstance(entry_price, (int,float)) or entry_price <= 0: return
    if side == "long":
        after = EARLY_CUT_LONG_SEC
    else:
        after = EARLY_CUT_SHORT_SEC
    _PENDING_EARLY_CUTS.append({
        "symbol": symbol, "side": side, "entry": float(entry_price),
        "ts": time.time(), "cut_after_sec": int(after),
        "buffer_pct": _get_buffer_pct(symbol, side)
    })

def _process_early_cuts() -> None:
    if not EARLY_CUT_ENABLED or not _PENDING_EARLY_CUTS: return
    now = time.time()
    keep: List[Dict[str, Any]] = []
    for item in list(_PENDING_EARLY_CUTS):
        if now - item["ts"] < item["cut_after_sec"]:
            keep.append(item); continue
        sym = item["symbol"]; side = item["side"]; entry = item["entry"]; buf = item["buffer_pct"]
        try:
            current = _last_price(sym)
            if current is None: keep.append(item); continue
            ret = _compute_return_pct(side, entry, current)
            adverse = (ret <= -buf)  # adverse if below -buffer%
            if adverse:
                msg = f"{sym} | CUT {side.upper()} | entry={_fmt_price(entry)} | now={_fmt_price(current)} | Δ%={ret:.3f} | threshold={-buf:.3f}%"
                print("[CUT]", msg); _send_telegram(msg)
                # close open trade if present
                tid = _LAST_TRADE_ID.get((sym, side))
                if tid: _close_trade(tid, "EARLY_CUT", current)
        except Exception as e:
            print("[early_cut] error:", e)
        # drop after evaluation
    _PENDING_EARLY_CUTS.clear()
    _PENDING_EARLY_CUTS.extend(keep)
...
def _level_flags_and_stoch(sym: str, price: float) -> Dict[str, Any]:
    ...
    for per in (10,20,50,200):
        ...
        if len(df1h) >= per: out[key] = _price_near_level(price, float(ema(df1h["close"], per).iloc[-1]))
        else: out[key] = 0
    # EMAs 1D
    for per in (20,50,100,200):
        key = f"near_ema{per}_1d"
        if len(df1d) >= per: out[key] = _price_near_level(price, float(ema(df1d["close"], per).iloc[-1]))
        else: out[key] = 0
    # top of book
    ...
    def _find_last_swing(df: pd.DataFrame, left=3, right=3) -> Tuple[Optional[float], Optional[float]]:
        ...
        if len(highs) < left+right+1: return None, None
        idx_end = len(highs) - right - 1
        hi_val = lo_val = None
        for i in range(idx_end - 1, left - 1, -1):
            if highs.iloc[i] == highs.iloc[i-left:i+right+1].max(): hi_val = float(highs.iloc[i]); break
        for i in range(idx_end - 1, left - 1, -1):
            if lows.iloc[i] == lows.iloc[i-left:i+right+1].min():  lo_val = float(lows.iloc[i]); break
        return hi_val, lo_val
    hi_s, lo_s = (None, None)
    if not df1h.empty: hi_s, lo_s = _find_last_swing(df1h)
    ...
    def _near_fib_flags(df_lohi: pd.DataFrame, lo_bars: int, hi_bars: int, key: str):
        if df_lohi.empty:
            out[f"near_{key}_min"] = 0; out[f"near_{key}_max"] = 0; out[f"in_{key}_zone"] = 0; return
        lo = float(df_lohi["low"].tail(lo_bars).min()); hi = float(df_lohi["high"].tail(hi_bars).max())
    ...
    # Stoch RSI
    if len(df1h) >= 30: k1h, d1h = stoch_rsi_kd(df1h["close"])
    else: k1h = d1h = float("nan")
    if len(df15) >= 30: k15, d15 = stoch_rsi_kd(df15["close"])
    else: k15 = d15 = float("nan")
...
def _calc_flows_for_windows(sym: str, windows_s: Dict[str,int]) -> Dict[str, float]:
    ...
    if tdf.empty or "time" not in tdf: return out
...
def _maybe_alert(symbol: str, side: str, entry_price: float, tp_pct: float, sl_pct: float, p_win: float = None, ev: float = None) -> None:
    if not _cooldown_ok(symbol, side): return
    if not (isinstance(entry_price, (int,float)) and isinstance(tp_pct, (int,float)) and isinstance(sl_pct, (int,float))): return
    if entry_price <= 0: return
    if side == "long":
        tp_price = entry_price * (1.0 + tp_pct/100.0); sl_price = entry_price * (1.0 - sl_pct/100.0)
    else:
        tp_price = entry_price * (1.0 - tp_pct/100.0); sl_price = entry_price * (1.0 + sl_pct/100.0)
    now_pt = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    msg = (
        f"{symbol} | {side.upper()} | entry={_fmt_price(entry_price)} | "
        f"TP={_fmt_price(tp_price)} | SL={_fmt_price(sl_price)} | "
        f"TP%={tp_pct:.2f} SL%={sl_pct:.2f}{p_txt}{ev_txt} | {now_pt}"
    )
    print("[ALERT]", msg); _send_telegram(msg)
    # Early cut scheduling
    _schedule_early_cut(symbol, side, entry_price)
    # Register trade
    _register_trade(symbol, side, entry_price, tp_pct, sl_pct, p_win, ev)

# -------------- Tick --------------
def _one_tick() -> None:
    windows_s = {
        "lookback_5m": 5*60, "lookback_15m": 15*60, "lookback_30min": 30*60,
        "lookback_1hr": 60*60, "lookback_4hr": 4*60*60,
    }
    cum = _cumulative_flows(windows_s)
    for sym in TRACK_SYMBOLS:
        try:
            price = _last_price(sym)
            if price is None: print(f"[skip] no price for {sym}"); continue
            flows = _calc_flows_for_windows(sym, windows_s)
            ratios = {k + "_ratio": (flows.get(k,0.0) / cum.get(k,0.0)) if abs(cum.get(k,0.0)) > 1e-9 else np.nan for k in windows_s}
            levels = _level_flags_and_stoch(sym, price)
            row = {
                "symbol": sym, "price": round(float(price), 8),
            ...
            long_setup  = setups.get("LONG"); short_setup = setups.get("SHORT")
...
def main() -> None:
    _load_symbol_setups(); _reload_symbol_setups_if_needed(force=True)
    ...
    while True:
        ...
        try:
            _one_tick()
            _process_early_cuts()
            _monitor_tp_sl_and_close()
        except Exception as e:
            print("[main] tick error:", e)
        dt = time.time() - t0
        if dt < PING_EVERY_SEC: time.sleep(PING_EVERY_SEC - dt)
