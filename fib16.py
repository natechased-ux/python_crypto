...
# Closer settings (if not already present)
CLOSER_POLL_S       = 60
CLOSER_LOOKBACK_H   = 24
CLOSER_MAX_PER_TICK = 30
...
import os as _os, time as _time, threading as _th

class TradeCloser(_th.Thread):
    """
    Watches recent 'open' trades in the log and stamps TP/SL as soon as
    price first-touches the level, using your existing fetch_candles(symbol, secs).
    Also fills basic MFE/MAE in percent since entry for that window.
    """
    def __init__(self, log_path: str, log_lock: _th.RLock,
                 poll_s: int = CLOSER_POLL_S,
                 lookback_h: int = CLOSER_LOOKBACK_H,
                 max_per_tick: int = CLOSER_MAX_PER_TICK):
    ...
    def _tick(self):
        if (not _os.path.exists(self.log_path)) or _os.path.getsize(self.log_path) == 0:
            return
        # Read under lock to avoid partial reads while the main loop writes
        with self.log_lock:
            df = _pd.read_csv(self.log_path, encoding="utf-8-sig")
        ...
        cutoff = now - _pd.to_timedelta(self.lookback_h, unit="h")

        # open = no outcome, or outcome empty/NaN
        out_series = df.get("outcome", _pd.Series("", index=df.index)).astype(str).str.upper()
        open_mask = (~out_series.isin(["TP", "SL"])) & (df["timestamp_utc"] >= cutoff)

        if not open_mask.any():
            return

        # Work on the most recent opens first, limit to max_per_tick
        open_df = df[open_mask].sort_values("timestamp_utc", ascending=False).head(self.max_per_tick).copy()

        updated_rows = []
        for idx, row in open_df.iterrows():
            try:
                res = self._close_one(row, now)
                if res is not None:
                    outcome, hit_tp, hit_sl, mfe_pct, mae_pct = res
                    df.at[idx, "outcome"] = outcome
                    df.at[idx, "hit_tp"]  = int(hit_tp)
                    df.at[idx, "hit_sl"]  = int(hit_sl)
                    # fill MFE/MAE percent (if present in schema)
                    if "mfe_pct" in df.columns: df.at[idx, "mfe_pct"] = mfe_pct
                    if "mae_pct" in df.columns: df.at[idx, "mae_pct"] = mae_pct
                    updated_rows.append(idx)
            except Exception as e:
                print("[CLOSER] row error:", e)
        ...
        if updated_rows:
            ...
            with self.log_lock:
                ...
                _os.replace(tmp_path, self.log_path)
            print(f"[CLOSER] updated {len(updated_rows)} rows → {self.log_path}")

    def _close_one(self, r, now_utc):
        """Return (outcome, hit_tp, hit_sl, mfe_pct, mae_pct) or None if still open."""
        ...
        if not (sym and side in {"LONG","SHORT"} and _np.isfinite(entry) and _np.isfinite(sl) and _np.isfinite(tp1) and _pd.notna(ts)):
            return None

        # Fetch minimal candles window: from just before trade time to now
        # Use your existing fetch_candles(symbol, seconds) helper
        # Prefer 60s candles for precise first-touch logic
        try:
            df1m = fetch_candles(sym, 60)
        except Exception as e:
            print("[CLOSER] fetch_candles failed", sym, e); return None
        if df1m is None or len(df1m) == 0:
            return None

        # Ensure datetime index/column is comparable
        time_col = "time" if "time" in df1m.columns else df1m.columns[0]
        # Many bot helpers already return a datetime index; be tolerant
        tvals = df1m[time_col] if time_col in df1m else df1m.index
        tvals = _pd.to_datetime(tvals, utc=True, errors="coerce")
        df1m = df1m.assign(_t=tvals)
        # Start a bit before to avoid missing first candle
        start_t = _pd.to_datetime(ts, utc=True) - _pd.to_timedelta(2, unit="min")
        w = df1m[df1m["_t"] >= start_t]
        if w.empty: 
            return None

        # High/Low columns
        try:
            high = _pd.to_numeric(w["high"], errors="coerce").values
            low  = _pd.to_numeric(w["low"], errors="coerce").values
        except KeyError:
            # If your fetch returns (time, open, high, low, close, volume)
            cols = list(w.columns)
            if len(cols) >= 5:
                high = _pd.to_numeric(w.iloc[:,2], errors="coerce").values
                low  = _pd.to_numeric(w.iloc[:,3], errors="coerce").values
            else:
                return None
        ...
        for hi, lo in zip(high, low):
            if side == "LONG":
                if _np.isfinite(hi) and hi >= tp1: 
                    hit_tp = True; outcome = "TP"; break
                if _np.isfinite(lo) and lo <= sl:
                    hit_sl = True; outcome = "SL"; break
            else:  # SHORT
                if _np.isfinite(lo) and lo <= tp1:
                    hit_tp = True; outcome = "TP"; break
                if _np.isfinite(hi) and hi >= sl:
                    hit_sl = True; outcome = "SL"; break

        if not outcome:
            return None  # still open

        # Compute simple MFE/MAE % since entry over observed window
        max_hi = _np.nanmax(high) if high.size else _np.nan
        min_lo = _np.nanmin(low)  if low.size  else _np.nan
        if side == "LONG":
            mfe_pct = float(max(0.0, (max_hi - entry) / entry * 100.0)) if _np.isfinite(max_hi) else _np.nan
            mae_pct = float(max(0.0, (entry - min_lo) / entry * 100.0)) if _np.isfinite(min_lo) else _np.nan
        else:
            mfe_pct = float(max(0.0, (entry - min_lo) / entry * 100.0)) if _np.isfinite(min_lo) else _np.nan
            mae_pct = float(max(0.0, (max_hi - entry) / entry * 100.0)) if _np.isfinite(max_hi) else _np.nan

        return (outcome, hit_tp, hit_sl, round(mfe_pct, 4), round(mae_pct, 4))
...
def log_trade(symbol, side, mode, entry, sl, tp1, tp2, stop_type, zone, zones, stars, reason, diag, now):
    ts_str = _fmt_utc(now if isinstance(now, datetime) else datetime.now(timezone.utc))
    init_log_file()
    ...
    ,
        "alert_sent": int(diag.get("alert_sent", 0))
    }
...
def _log_only(reason: str, diag: dict, entry_v: float, sl_v: float, tp_v: float):
    """Write a log-only (non-alert) row safely with explicit values (no reliance on caller locals)."""
    try:
        _diag = dict(diag or {})
        _diag["alert_sent"]      = 0
        _diag["suppressed"]      = 1
        _diag["suppress_reason"] = reason
        # fallbacks if somehow None/NaN
        ev = entry_v if entry_v is not None else float("nan")
        sv = sl_v    if sl_v    is not None else ev
        tv = tp_v    if tp_v    is not None else ev
        log_trade(
            product_id, side, mode,
            ev, sv, tv, None,
            "LOGONLY", zone_source, zones_list, stars if 'stars' in locals() else "★☆☆",
            liq_reason if 'liq_reason' in locals() else "", _diag, now
        )
    except Exception as _e:
        print("[log_only] error:", _e)
...
def main_loop():
    ...
    # --- start the live closer once ---
    try:
        global _CLOSER_THREAD
    except NameError:
        _CLOSER_THREAD = None
    try:
        # ensure a log lock exists
        try:
            LOG_LOCK
        except NameError:
            import threading
            LOG_LOCK = threading.RLock()
        if _CLOSER_THREAD is None or not _CLOSER_THREAD.is_alive():
            _CLOSER_THREAD = TradeCloser(
                LOG_PATH, LOG_LOCK,
                poll_s=CLOSER_POLL_S,
                lookback_h=CLOSER_LOOKBACK_H,
                max_per_tick=CLOSER_MAX_PER_TICK,
            )
            _CLOSER_THREAD.start()
            print("[CLOSER] started")
    except Exception as e:
        print("[CLOSER] start error:", e)
    # ----------------------------------
