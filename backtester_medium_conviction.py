...
# -*- coding: utf-8 -*-
"""
High-Conviction Backtester (MEDIUM) — more trades, still structured

Changes vs strict version:
- ADX default: 25 (grid defaults 22,25)
- FVG off by default (can enable with --use_fvg)
- Stoch RSI confirmation relaxed:
    * Long: K > D and K < 50
    * Short: K < D and K > 50
- Confirmation window: 12h (vs 6h)
- Removed one-trade-per-day cap
- Secondary triggers (in addition to StochRSI):
    * 1H swing breakout in trend direction (break prior 20-bar extreme)
    * 1H pullback to EMA20/EMA50 in trend direction

Keeps:
- ATR-based TP/SL simulation
- MFE/MAE logging
- Coinbase loader
- --out_dir for portable CSV output

Run:
  py backtester_medium_conviction.py --out_dir .\output
"""
...
def atr(df: pd.DataFrame, length: int = 14) -> pd.Series:
    tr = true_range(df); return tr.ewm(alpha=1/length, adjust=False).mean()

def macd(series: pd.Series, fast=12, slow=26, signal=9) -> Tuple[pd.Series, pd.Series, pd.Series]:
    fast_ema = ema(series, fast); slow_ema = ema(series, slow)
    macd_line = fast_ema - slow_ema; signal_line = ema(macd_line, signal)
    hist = macd_line - signal_line; return macd_line, signal_line, hist
...
def stoch_rsi(series: pd.Series, rsi_len=14, stoch_len=14, k=3, d=3) -> Tuple[pd.Series, pd.Series]:
    ...
    min_r = r.rolling(stoch_len).min(); max_r = r.rolling(stoch_len).max()
    stoch = (r - min_r) / ((max_r - min_r) + 1e-12)
    k_line = stoch.rolling(k).mean() * 100.0; d_line = k_line.rolling(d).mean()
...
def fetch_coinbase_candles(product_id: str, start: str, end: str, granularity: int = 3600, pause: float = 0.25) -> pd.DataFrame:
    ...
    frames = []; headers = {"User-Agent": "hc-backtester/1.0"}
    t0 = start_ts
    while t0 < end_ts:
        ...
        if data:
            ...
            frames.append(df)
        time.sleep(pause); t0 = t1
    if not frames: raise ValueError(f"No data from Coinbase for {product_id} in {start}..{end}")
...
class Params:
    ...
    tp_mult: float = 2.0
    sl_mult: float = 1.5
    confirm_hours: int = 12  # relaxed
    one_trade_per_day: bool = False  # relaxed
    use_fvg: bool = True  # relaxed default
    use_whale: bool = False
    whale_max_distance_pct: float = 0.03
    # Secondary trigger params
    swing_lookback: int = 20  # bars
    ema_fast: int = 20
    ema_slow: int = 50
...
def compute_bias_and_momentum(htf: pd.DataFrame, p: Params) -> pd.DataFrame:
    ...
    _, _, hist = macd(df["close"]); df["macd_hist"] = hist
    df["adx"] = adx(df); df["rsi"] = rsi(df["close"])
...
def secondary_triggers(ltf: pd.DataFrame, p: Params) -> pd.DataFrame:
    df = ltf.copy()
    df["ema20"] = ema(df["close"], p.ema_fast)
    df["ema50"] = ema(df["close"], p.ema_slow)
    df["prev_high20"] = df["high"].shift(1).rolling(p.swing_lookback).max()
    df["prev_low20"]  = df["low"].shift(1).rolling(p.swing_lookback).min()
    df["breakout_long"] = df["high"] > df["prev_high20"]
    df["breakout_short"] = df["low"] < df["prev_low20"]
    df["pullback_long"] = (df["low"] <= df["ema20"]) | (df["low"] <= df["ema50"])
    df["pullback_short"] = (df["high"] >= df["ema20"]) | (df["high"] >= df["ema50"])
    return df[["ema20","ema50","breakout_long","breakout_short","pullback_long","pullback_short"]]

def simulate_trade_with_excursions(ltf: pd.DataFrame, entry_idx: int, side: str, entry_price: float,
                                   atr_val: float, p: Params) -> Tuple[pd.Timestamp, float, str, float, float, float, float, float]:
    if side == "long":
...
def run_backtest(run_id: str,
                 ...
                 grid: Dict,
                 out_dir: str,
                 use_fvg: bool) -> pd.DataFrame:
    os.makedirs(out_dir, exist_ok=True)
    results = []
    grid_items = dict(grid)
    grid_items.setdefault("adx", [22.0, 25.0])
    grid_items.setdefault("tp_mult", [2.0])
    grid_items.setdefault("sl_mult", [1.25, 1.5])
    ...
    for adx_min in grid_items["adx"]:
        for tp_mult in grid_items["tp_mult"]:
            for sl_mult in grid_items["sl_mult"]:
                ...
                params = Params(adx_min=adx_min, tp_mult=tp_mult, sl_mult=sl_mult, use_fvg=use_fvg)
                ...
                for symbol in symbols:
                    ...
                    sec = secondary_triggers(ltf, params); ltf = pd.concat([ltf, sec], axis=1)
                    ...
                    for idx in range(2, len(htf)-1):
                        ...
                        if side == "long":
                            trig = window["stoch_long_ok"] | window["breakout_long"] | window["pullback_long"]
                        else:
                            trig = window["stoch_short_ok"] | window["breakout_short"] | window["pullback_short"]
                        ...
                        exit_time, exit_price, outcome, rr, max_high, min_low, mfe_pct, mae_pct = simulate_trade_with_excursions(
                            ltf, entry_pos, side, entry_price, atr_val, params
                        )
...
def main():
    ...
    DEFAULT_GRID = {"adx":[22,25], "tp_mult":[2.0], "sl_mult":[1.25,1.5]}
    DEFAULTS = {
        "data_source": "coinbase",
        "symbols": DEFAULT_SYMBOLS,
        "start": start_def, "end": end_def,
        "grid": DEFAULT_GRID,
        "run_id": "hc_medium_auto",
        "out_dir": os.getcwd(),
        "use_fvg": True,
    }
    ...
    ap.add_argument("--grid", type=str, help='JSON like {"adx":[22,25],"tp_mult":[2.0],"sl_mult":[1.25,1.5]}')
    ...
    ap.add_argument("--use_fvg", action="store_true", help="Require FVG confluence (off by default)")
    args = ap.parse_args()
    ...
    use_fvg = args.use_fvg or DEFAULTS["use_fvg"]
    os.makedirs(out_dir, exist_ok=True)

    print("=== Medium-Strict Backtester ===")
    ...
    print(f"Use FVG     : {use_fvg}")

    leaderboard = run_backtest(run_id, args.data_dir, data_source, symbols, start, end, grid, out_dir, use_fvg)
