"""
Viv (Blockchained Snipers) HTF Bias → Python Telegram Alerts (HTF-only v1)
---------------------------------------------------------------------
High-timeframe-only variant:
- Uses 1D for primary trend bias
- Uses 4H for all structure, sweep, FVG/OB, OTE, volume confirmation
...
LOOKBACK_BARS = {"1d": 220, "4h": 480, "1h": 1500}
...
# Volume spike (applies to 4H series)
VOL_LOOKBACK = 32  # number of 4H bars to look back for percentile
VOL_PERCENTILE = 10

# Sweep detection window (in 4H bars)
SWEEP_LOOKBACK = 48
...
VWAP_SOFT_GATE = False  # disabled for HTF-only (we'll keep PVWAP as a metric)
...
# PVWAP (session-reset VWAP) — optional metric
PVWAP_ENABLED = True
PVWAP_RESETS_LOCAL = ["00:00", "06:30"]

TZ = pytz.timezone("America/Los_Angeles")
STATE_PATH = "viv_state_htf.json"
...
def last_major_swing(df4h: pd.DataFrame) -> Tuple[float, float]:
    sh, sl = swing_points(df4h, left=3, right=3)
    idx = len(df4h) - 2
    last_high_idx = np.where(sh[:idx])[0]
    last_low_idx = np.where(sl[:idx])[0]
    if len(last_high_idx) == 0 or len(last_low_idx) == 0:
        return float(df4h['low'].iloc[-50:-1].min()), float(df4h['high'].iloc[-50:-1].max())
    h_idx = last_high_idx[-1]
    l_before_high = last_low_idx[last_low_idx < h_idx]
    if len(l_before_high):
        l_idx = l_before_high[-1]
        return float(df4h['low'].iloc[l_idx]), float(df4h['high'].iloc[h_idx])
...
class VivScanner:
    ...
    def eligible(self, symbol: str) -> Optional[Setup]:
        ...
        # ----- HTF structure checks (ALL on 4H) -----
        # Range & sweep on 4H
        rng = find_range(h4)
        sweep = detect_sweep(h4, rng)
        has_manip = (sweep['sweep_low'] and bias == 'long') or (sweep['sweep_high'] and bias == 'short')

        # Equal-liquidity levels on 4H
        ...
        # Volume spike on 4H
        vol_ok = volume_spike(h4)

        # Swing/OTE zone from 4H swings
        swing_lo, swing_hi = last_major_swing(h4)
        ...
        # (stricter filter to force HTF setups)
        h4_latest = h4.iloc[-1]
        ...
        if bias == "long":
            # require 4H close above EMA_SLOW for confirmation
        ...
        # Core HTF gate (polarity-correct)
        #core_ok = in_or_near_zone and has_manip and has_struct
        core_ok = in_or_near_zone and has_manip
        if not core_ok:
            return None

        # Risk / targets (derived from 4H swings)
        if bias == 'long':
        ...
        # Confluence tags
        confl = [f"bias:{bias}", f"PVWAP:{vwap_dir}", poc_confluence]
        if rng: confl.append("range4h")
...
def format_alert(s: Setup) -> str:
    ...
    lines = [
        f"*Viv-Style HTF Setup* — *{md_escape(s.symbol)}* ({s.direction.upper()}) {stars}",
        f"Score: {score:.2f}",
        f"Time: {ts}",
