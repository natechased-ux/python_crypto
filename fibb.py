...
# === Simple alert formatter (added) ===

def _format_price_dynamic(x):
    try:
        val = float(x)
        if val < 1:
            return f"{val:.6f}"
...
# === Dynamic price formatter (added) ===
def _dynamic_decimals(x: float) -> int:
    try:
        v = abs(float(x))
    except Exception:
        return 4
...
def _fmt_dyn(x):
    try:
        dec = _dynamic_decimals(x)
        return f"{float(x):.{dec}f}"
    except Exception:
        return str(x)

def format_simple_alert(symbol: str, entry: float, tp: float, sl: float) -> str:
    sym = str(symbol).replace("-", "/").upper()
    return f"{sym} | Entry: ${_format_price_dynamic(entry)} | TP: ${_format_price_dynamic(tp)} | SL: ${_format_price_dynamic(sl)}"


# === Ensure trade log columns (added) ===
import pandas as _pd, numpy as _np, os as _os


# === Autotuner features writer (added) ===
import threading as _threading, time as _time, pandas as _pd

_AUTOTUNER_IN  = "live_trade_log_fib6.csv"
_AUTOTUNER_OUT = "autotuner_features.csv"

# Choose a conservative feature set; include if columns exist.
_FEATURE_COLUMNS = [
    # identifiers/timing
    "timestamp_utc","symbol","side","signal_time_pst","signal_time_utc","entry_price","entry",
    # core levels
    "tp","sl","take_profit","stop_loss",
    # outcomes
    "hit_tp","hit_sl","exit_reason","exit_time_utc","exit_price","r_multiple","r",
    # mae/mfe
    "mae_pct","mfe_pct",
    # stoch rsi (examples)
    "stoch15m_k","stoch15m_d","stoch1h_k","stoch1h_d","stoch6h_k","stoch6h_d","stoch1d_k","stoch1d_d",
    "stoch15m_cross_up","stoch15m_cross_dn","stoch1h_cross_up","stoch1h_cross_dn",
    # optional EMA guidance etc.
    "ema1h_10","ema1h_20","ema1h_50","ema1h_200","ema1d_200"
]

def _write_autotuner_snapshot():
    try:
        if not os.path.exists(_AUTOTUNER_IN):
            return
        df = _pd.read_csv(_AUTOTUNER_IN, encoding="utf-8-sig")
        keep = [c for c in _FEATURE_COLUMNS if c in df.columns]
        if not keep:
            return
        out = df[keep].copy()
        # Drop rows without entry or side if present to keep data clean
        for must in ["entry","entry_price","side"]:
            if must in out.columns:
                out = out[~out[must].isna()]
        # Deduplicate by timestamp+symbol+side if available
        key_cols = [c for c in ["timestamp_utc","symbol","side"] if c in out.columns]
        if key_cols:
            out = out.drop_duplicates(subset=key_cols, keep="last")
        out.to_csv(_AUTOTUNER_OUT, index=False, encoding="utf-8-sig")
    except Exception:
        # Silent guard: this should never crash the main loop
        pass

def start_autotuner_export_daemon(interval_seconds: int = 60):
    def _loop():
        while True:
            _write_autotuner_snapshot()
            _time.sleep(interval_seconds)
    t = _threading.Thread(target=_loop, daemon=True, name="AutotunerFeaturesWriter")
    t.start()
    return t

def ensure_trade_log_columns(csv_path: str):
    try:
        ...
        return
    added = False
    for c in ['stoch15m_k', 'stoch15m_d', 'stoch15m_cross_up', 'stoch15m_cross_dn', 'stoch15m_overbought', 'stoch15m_oversold', 'stoch1h_k', 'stoch1h_d', 'stoch1h_cross_up', 'stoch1h_cross_dn', 'stoch1h_overbought', 'stoch1h_oversold', 'stoch6h_k', 'stoch6h_d', 'stoch1d_k', 'stoch1d_d', 'mfe_r_5m', 'mae_r_5m', 'mfe_pct_5m', 'mae_pct_5m', 'c_mfe_r_5m', 'c_mae_r_5m', 'c_mfe_pct_5m', 'c_mae_pct_5m', 'mfe_r_15m', 'mae_r_15m', 'mfe_pct_15m', 'mae_pct_15m', 'c_mfe_r_15m', 'c_mae_r_15m', 'c_mfe_pct_15m', 'c_mae_pct_15m', 'mfe_r_30m', 'mae_r_30m', 'mfe_pct_30m', 'mae_pct_30m', 'c_mfe_r_30m', 'c_mae_r_30m', 'c_mfe_pct_30m', 'c_mae_pct_30m', 'mfe_r_1h', 'mae_r_1h', 'mfe_pct_1h', 'mae_pct_1h', 'c_mfe_r_1h', 'c_mae_r_1h', 'c_mfe_pct_1h', 'c_mae_pct_1h', 'mfe_r_2h', 'mae_r_2h', 'mfe_pct_2h', 'mae_pct_2h', 'c_mfe_r_2h', 'c_mae_r_2h', 'c_mfe_pct_2h', 'c_mae_pct_2h', 'mfe_r_4h', 'mae_r_4h', 'mfe_pct_4h', 'mae_pct_4h', 'c_mfe_r_4h', 'c_mae_r_4h', 'c_mfe_pct_4h', 'c_mae_pct_4h', 'mfe_r_6h', 'mae_r_6h', 'mfe_pct_6h', 'mae_pct_6h', 'c_mfe_r_6h', 'c_mae_r_6h', 'c_mfe_pct_6h', 'c_mae_pct_6h', 'mfe_r_12h', 'mae_r_12h', 'mfe_pct_12h', 'mae_pct_12h', 'c_mfe_r_12h', 'c_mae_r_12h', 'c_mfe_pct_12h', 'c_mae_pct_12h', 'mfe_r_24h', 'mae_r_24h', 'mfe_pct_24h', 'mae_pct_24h', 'c_mfe_r_24h', 'c_mae_r_24h', 'c_mfe_pct_24h', 'c_mae_pct_24h']:
...
EXITS_LEARN_WINDOW_HOURS = 720
EXITS_MIN_SAMPLES = 40
EXITS_EWMA_TAU_HOURS = 72
EXITS_TP_QUANT = 0.65
EXITS_SL_QUANT = 0.60
EXITS_LEARN_EVERY_SEC = 3*3600
EXPLORATION_RATE = 0.10
EXPLORATION_TP_JITTER = 0.3
EXPLORATION_SL_JITTER = 0.2
...
def check_signal(product_id: str, cooldowns: dict, last_alert_key: dict):
    ...
    # -------------
    # Apply learned exits (quantile/EWMA) with exploration when applicable
    try:
        _fn = globals().get("_apply_learned_exits")
        if _fn:
            sl_price, tp1_price, stop_type = _fn(product_id, side, mode, zone_source, entry, sl_price, tp1_price, liq_diag)
    except Exception as _e:
        print("[exits] apply error:", _e)


    # Alert message
    # -------------
    liq_line = f"ΔCVD:{fmt_dollars(liq_diag.get('delta_usd',0))} | nearB:{fmt_dollars(b_near)} nearA:{fmt_dollars(a_near)} | {liq_reason} ✅ ({liq_diag.get('source','')})"
    msg = (
        f"{'🔻' if side=='SHORT' else '🚀'} *{side}* on *{product_id}* — {mode} — Strength: *{stars}*\n"
    ...
    # --- Flow annotation (printed + logged) ---
    qG = abs_delta_q_global if abs_delta_q_global is not None else "-"
    qC = abs_delta_q_coin   if abs_delta_q_coin   is not None else "-"
    msg += f"\nFlow: |Δ|/thr={delta_ratio:.2f} • qG={qG} qC={qC} • flow_ok={'Y' if flow_aligned else 'N'} • book_ok={'Y' if book_aligned else 'N'}"
    if mode == "REVERSAL" and (abs_delta_q_global is not None) and abs_delta_q_global >= 3:
        msg += " • QG3+ (rev sweet-spot)"
...
def main_loop():
    ...
    while True:
        ...
        for coin in COINS:
            try:
                alert = check_signal(coin, cooldowns, STATE.get("last_alert_key", {}))
                if alert:
...
# === Autotuner feature writer (added) ===
def write_autotuner_features(trade: dict, csv_path="autotuner_features.csv"):
    import pandas as _pd, os as _os
    # trade is expected to be dict of all trade log fields (including outcome if available)
    try:
        df_new = _pd.DataFrame([trade])
        if _os.path.exists(csv_path):
            df_old = _pd.read_csv(csv_path, encoding="utf-8-sig")
            df_all = _pd.concat([df_old, df_new], ignore_index=True)
        else:
            df_all = df_new
        df_all.to_csv(csv_path, index=False, encoding="utf-8-sig")
    except Exception as e:
        print("write_autotuner_features failed", e)

if __name__ == "__main__":
    # Start periodic export for autotuner features
    try:
        start_autotuner_export_daemon(60)
    except Exception:
        pass
    # Ensure CSV has all analytics columns for backfills/autotuner
    try:
        ensure_trade_log_columns("live_trade_log_fib6.csv")
    except Exception:
        pass
...
def _profile_for_row(row):
    z = str(row.get("zone","")).upper()
    mode = str(row.get("mode","")).upper()
    stop_type = str(row.get("stop_type","")).upper()
    if "SWING12HSIMPLE" in stop_type:
        return "SWING"
    if z.startswith("EMA") and mode == "CONTINUATION":
        return "C13_CONT"
    if z.startswith("EMA") and mode != "CONTINUATION":
        return "C13_REV_EMA"
    return "C13_REV_ZONE"

def _tier_for_row(row, coin_thr):
    try:
        du = float(row.get("delta_usd", 0.0))
        abs_du = abs(du)
        k1h = float(row.get("stoch1h_k", float("nan")))
        d1h = float(row.get("stoch1h_d", float("nan")))
    except Exception:
        du = 0.0; abs_du = 0.0; k1h = float("nan"); d1h = float("nan")
    side = str(row.get("side","")).upper()
    if side == "LONG":
        stoch_ok = (k1h <= 20.0) or (d1h <= 20.0)
        flow_ok = (abs_du >= 1.3*coin_thr) and (du > 0)
    else:
        stoch_ok = (k1h >= 80.0) or (d1h >= 80.0)
        flow_ok = (abs_du >= 1.3*coin_thr) and (du < 0)
    if stoch_ok and flow_ok: return "A"
    if stoch_ok or flow_ok:  return "B"
    return "C"

def _learn_exits_profiles(state: dict):
    import pandas as _pd, numpy as _np, os
    if not os.path.exists(LOG_PATH): return
    df = _pd.read_csv(LOG_PATH)
    if "timestamp_utc" not in df.columns: return
    df["timestamp_utc"] = _pd.to_datetime(df["timestamp_utc"], utc=True, errors="coerce")
    cutoff = _now_utc_pd() - _pd.to_timedelta(EXITS_LEARN_WINDOW_HOURS, unit="h")
    d = df[df["timestamp_utc"] >= cutoff].copy()
    if d.empty: return
    # risk pct
    d["risk_pct"] = _pd.to_numeric((d.get("entry") - d.get("sl")).abs() / d.get("entry"), errors="coerce")
    # MFE/MAE in R
    if "mfe_r_12h" in d.columns and "mae_r_12h" in d.columns:
        d["mfe_r12"] = _pd.to_numeric(d["mfe_r_12h"], errors="coerce")
        d["mae_r12"] = _pd.to_numeric(d["mae_r_12h"], errors="coerce").abs()
    else:
        d["mfe_r12"] = _pd.to_numeric(d.get("mfe_pct"), errors="coerce")/100.0 / d["risk_pct"]
        d["mae_r12"] = _pd.to_numeric(d.get("mae_pct"), errors="coerce")/100.0 / d["risk_pct"]
    # EWMA weights
    age_hours = (_now_utc_pd() - d["timestamp_utc"]).dt.total_seconds() / 3600.0
    w = _np.exp(-age_hours / float(EXITS_EWMA_TAU_HOURS))
    d["_w"] = w
    # coin flow threshold proxy
    coin_thr = d.groupby("symbol")["delta_usd"].apply(lambda s: _np.nanmean(_np.abs(_pd.to_numeric(s, errors="coerce"))))
    d["_coin_thr"] = d["symbol"].map(coin_thr)
    # profiles/tiers
    d["_profile"] = d.apply(_profile_for_row, axis=1)
    d["_tier"] = d.apply(lambda r: _tier_for_row(r, r.get("_coin_thr", _np.nan)), axis=1)
    out = {"updated_at": int(_now_utc_pd().timestamp()), "profiles": {}}
    def wq(vals, wts, q):
        import numpy as _np
        order = _np.argsort(vals)
        v = _np.array(vals)[order]; w = _np.array(wts)[order]
        c = _np.cumsum(w) / _np.sum(w)
        return float(_np.interp(q, c, v))
    for (prof, tier), g in d.groupby(["_profile","_tier"]):
        g = g[["mfe_r12","mae_r12","_w"]].dropna()
        if len(g) < EXITS_MIN_SAMPLES: continue
        TP_R = wq(g["mfe_r12"].values, g["_w"].values, EXITS_TP_QUANT)
        SL_R = wq(g["mae_r12"].values, g["_w"].values, EXITS_SL_QUANT)
        out.setdefault("profiles", {}).setdefault(prof, {})[tier] = {"TP_R": round(TP_R,3), "SL_R": round(SL_R,3), "n": int(len(g))}
    for prof, g in d.groupby("_profile"):
        g = g[["mfe_r12","mae_r12","_w"]].dropna()
        if len(g) < EXITS_MIN_SAMPLES: continue
        out.setdefault("profiles", {}).setdefault(prof, {})["*"] = {
            "TP_R": float(g["mfe_r12"].quantile(EXITS_TP_QUANT)),
            "SL_R": float(g["mae_r12"].quantile(EXITS_SL_QUANT)),
            "n": int(len(g))
        }
    _safe_json_write(EXITS_JSON_PATH, out)

def _apply_learned_exits(product_id, side, mode, zone_source, entry, sl_price, tp1_price, liq_diag):
    prof = _profile_for_row({"zone": zone_source, "mode": mode, "stop_type": liq_diag.get("stop_type",""), "side": side})
    stype = str(liq_diag.get("stop_type","")).upper()
    if stype == "LIQ" and not "SWING" in stype:
        return sl_price, tp1_price, stype
    data = _safe_json_read(EXITS_JSON_PATH)
    prof_tiers = data.get("profiles", {}).get(prof, {})
    coin_thr = float(liq_diag.get("abs_delta_thr", 0.0)) if "abs_delta_thr" in liq_diag else abs(float(liq_diag.get("delta_usd", 0.0)))
    tier = _tier_for_row({"delta_usd": liq_diag.get("delta_usd",0.0), "stoch1h_k": liq_diag.get("stoch1h_k", float("nan")),
                          "stoch1h_d": liq_diag.get("stoch1h_d", float("nan")), "side": side}, coin_thr)
    params = prof_tiers.get(tier) or prof_tiers.get("*")
    if not params:
        return sl_price, tp1_price, stype
    TP_R = float(params.get("TP_R", 0.0)); SL_R = float(params.get("SL_R", 0.0))
    import random
    if random.random() < EXPLORATION_RATE:
        TP_R += random.uniform(-EXPLORATION_TP_JITTER, EXPLORATION_TP_JITTER)
        SL_R += random.uniform(-EXPLORATION_SL_JITTER, EXPLORATION_SL_JITTER)
        liq_diag["explore"] = 1
        liq_diag["tp_r_used"] = round(TP_R,3); liq_diag["sl_r_used"] = round(SL_R,3)
    risk = abs(entry - sl_price)
    if risk <= 0: return sl_price, tp1_price, stype
    if str(side).upper() == "LONG":
        new_sl = entry - SL_R * risk
        new_tp = entry + TP_R * risk
    else:
        new_sl = entry + SL_R * risk
        new_tp = entry - TP_R * risk
    return new_sl, new_tp, "LEARNED"


def send_alert_simple(symbol, entry, tp, sl, send_fn=None):
    msg = format_simple_alert(symbol, entry, tp, sl)
    if send_fn is not None:
        return send_fn(msg)
    try:
        send_telegram_message(msg)  # if the project exposes this
    except Exception:
        pass
