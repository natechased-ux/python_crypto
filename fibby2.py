...
class TradeCloser(_th.Thread):
    ...
    def _close_one(self, r, now_utc):
        ...
        # Guards: require valid entry/sl/tp1 and positive risk
        if not (_np.isfinite(entry) and _np.isfinite(sl) and _np.isfinite(tp1)):
            return None
        risk = abs(entry - sl)
        if (not _np.isfinite(risk)) or (risk <= MIN_RISK_EPS) or (abs(tp1 - entry) <= MIN_RISK_EPS):
            return None
        ...
        w_all = df1m[df1m["_t"] >= start_t]
        if w_all.empty:
            return None
        arm_ts = _pd.to_datetime(ts, utc=True) + _pd.to_timedelta(CLOSER_ARM_DELAY_SEC, unit="s")
        w = w_all[w_all["_t"] > arm_ts]
        if w.empty:
            return None
        if w.empty: 
            return None
