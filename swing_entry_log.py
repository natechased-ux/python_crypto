...
class CandleCache:
    def __init__(self, ttl_minutes=10):
        self.cache = {}  # { (symbol, tf): {"data": df, "timestamp": datetime} }
        self.ttl = timedelta(minutes=ttl_minutes)

    def get(self, symbol, granularity, lookback_days):
        key = (symbol, granularity)
        now = datetime.utcnow()

        # If in cache and fresh → return
        ...
        df = fetch_candles(symbol, granularity, lookback_days=lookback_days)
        if df is not None:
            self.cache[key] = {"data": df, "timestamp": now}
        return df


# Create a global cache instance
candle_cache = CandleCache(ttl_minutes=10)
...
def check_signals():
    ...
    for symbol in COINS:
        ...
        df_6h = candle_cache.get(symbol, GRANULARITY_6H, lookback_days=30)
        if df_6h is None or len(df_6h) < 30:
            continue

        df_6h = calculate_indicators(df_6h)
        last_row = df_6h.iloc[-1]
        entry_price = last_row["close"]

        if np.isnan(last_row["adx"]) or np.isnan(last_row["rsi"]) or np.isnan(last_row["macd_diff"]):
            continue

        # 1H and 1D from cache
        df_1h = candle_cache.get(symbol, GRANULARITY_1H, lookback_days=12)
        df_1d = candle_cache.get(symbol, GRANULARITY_1D, lookback_days=300)
        ...
        ema200_1h, ema200_1d = hourly_emas["ema200"], daily_emas["ema200"]

        # === ATR Volatility filter ===
        atr = (df_6h['high'] - df_6h['low']).rolling(14).mean()
        avg_atr = atr.mean()
        if atr.iloc[-1] <= 0.8 * avg_atr:
            print(f"⛔ {symbol.upper()} skipped — Low volatility.")
            continue

        # === EMA cluster filter ===
        above, below = get_nearby_emas(entry_price, {**hourly_emas, **daily_emas}, max_percent_diff=1.0)
        if len(above) >= 2 or len(below) >= 2:
            print(f"⛔ {symbol.upper()} skipped — EMA cluster congestion.")
            continue

        # === Swing highs/lows for SL ===
        swings_high = [(i, df_6h["high"].iloc[i]) for i in range(len(df_6h)) if find_swing_high(df_6h, i)]
        swings_low  = [(i, df_6h["low"].iloc[i]) for i in range(len(df_6h)) if find_swing_low(df_6h, i)]
        ...
        if (
            ...
            last_row["macd_diff"] > 0 and
            last_row["rsi"] > 55 and
            entry_price > ema200_1h and entry_price > ema200_1d
        ):
            swings = [p for j, p in swings_low if j < len(df_6h) - 2]
            if swings:
                sl = swings[-1]
                risk = entry_price - sl
                ...
                threading.Thread(
                    target=monitor_stoch_confirmation,
                    args=(symbol, "long", entry_price, tp, sl, signal_time)
                ).start()

        # === SHORT SETUP ===
        elif (
            last_row["adx"] > 23 and
            last_row["minus_di"] > last_row["plus_di"] and
            last_row["macd_diff"] < 0 and
            last_row["rsi"] < 45 and
            entry_price < ema200_1h and entry_price < ema200_1d
        ):
            swings = [p for j, p in swings_high if j < len(df_6h) - 2]
            if swings:
                sl = swings[-1]
                risk = sl - entry_price
                tp = entry_price - TP_MULTIPLIER * risk
                ...
                threading.Thread(
                    target=monitor_stoch_confirmation,
                    args=(symbol, "short", entry_price, tp, sl, signal_time)
                ).start()
...
def monitor_stoch_confirmation(symbol, side, entry, tp, sl, signal_time):
    ...
    while datetime.utcnow() < timeout:
        # === Use persistent cache for candles ===
        df_1h = candle_cache.get(symbol, GRANULARITY_1H, lookback_days=12)
        df_1d = candle_cache.get(symbol, GRANULARITY_1D, lookback_days=300)
        ...
        if side == "long":
            trend_ok = entry_price > daily_emas['ema200'] and entry_price > hourly_emas['ema200']
        else:
            trend_ok = entry_price < daily_emas['ema200'] and entry_price < hourly_emas['ema200']

        # === ATR Volatility filter ===
        atr = (df_1h['high'] - df_1h['low']).rolling(14).mean()
        avg_atr = atr.mean()
        volatility_ok = atr.iloc[-1] > 0.8 * avg_atr

        # === EMA cluster avoidance ===
        above, below = get_nearby_emas(entry_price, {**hourly_emas, **daily_emas}, max_percent_diff=1.0)
        if (len(above) >= 2 and side == "long") or (len(below) >= 2 and side == "short"):
            print(f"⛔ {symbol.upper()} skipped due to EMA cluster near price.")
            if symbol in open_confirmations:
                del open_confirmations[symbol]
            return

        # === Stoch RSI Confirmation ===
        stoch = StochRSIIndicator(df_1h["close"]).stochrsi_k().dropna() * 100
        stoch_d = StochRSIIndicator(df_1h["close"]).stochrsi_d().dropna() * 100
        if stoch.empty or stoch_d.empty:
            time.sleep(300)
            continue

        k, d = stoch.iloc[-1], stoch_d.iloc[-1]
        prev_k, prev_d = stoch.iloc[-2], stoch_d.iloc[-2]

        if side == "long":
            stoch_ok = prev_k < 20 and k > d and k > prev_k and d > prev_d
        else:
            stoch_ok = prev_k > 80 and k < d and k < prev_k and d < prev_d

        # === Final trade confirmation ===
        if trend_ok and volatility_ok and stoch_ok:
            confirmed_entry_price = get_live_price(symbol)
            if confirmed_entry_price is None:
                print(f"❌ Skipping alert for {symbol.upper()} — no live price.")
                if symbol in open_confirmations:
                    del open_confirmations[symbol]
                return

            ema_msg = get_weighted_ema_guidance(confirmed_entry_price, hourly_emas, daily_emas)
            atr_val = round(atr.iloc[-1], 2)
            vol_status = "✅ OK" if volatility_ok else "⚠ Low"
            trend_status = "✅ Strong" if trend_ok else "⚠ Weak"

            alert_msg = (
                f"✅ *{side.upper()} CONFIRMED* on {symbol.upper()}\n"
                f"Entry: {confirmed_entry_price:.4f} | TP: {tp:.4f} | SL: {sl:.4f}\n"
                f"Trend: {trend_status} | ATR: {atr_val} ({vol_status})\n"
                f"Signal Time (PST): {signal_time}\n"
                f"{ema_msg}"
            )

            send_telegram_alert(alert_msg)
            log_live_trade(symbol, side, confirmed_entry_price, tp, sl, signal_time, hourly_emas, daily_emas)
            if symbol in open_confirmations:
                del open_confirmations[symbol]
            break
