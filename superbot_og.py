...
MODEL_PATH_TREND = Path("trend_follow_model.pkl")
MODEL_PATH_MEANREV = Path("mean_reversion_model.pkl")
...
MEANREV_THRESHOLD = 0.7

def format_price(v):
    return f"${v:.2f}" if v >= 100 else f"${v:.4f}" if v >= 1 else f"${v:.6f}"

def fetch_data(symbol, granularity='900', limit=8000):
    """
...
def get_historical_win_rate(current_df, label_func, symbol):
    ...
    try:
        ...
        df = fetch_data(symbol, granularity='900', limit=4000)  # ~83 days
...
def label_mean_reversion(df):
    ...
    for i in range(len(df)):
        if i + LOOKAHEAD_CANDLES >= len(df): labels.append(np.nan); continue
        overbought = df["rsi"].iloc[i] > 75 and df["close"].iloc[i] > df["bb_upper"].iloc[i]
        oversold = df["rsi"].iloc[i] < 25 and df["close"].iloc[i] < df["bb_lower"].iloc[i]
...
def retrain_if_needed():
    ...
    historical_data = {}
    for coin in COINS:
        df = fetch_data(coin, granularity='900', limit=8000)  # 900 for M15 bot
...
def send_alert(strategy, symbol, side, live_price, tp, sl, prob, hist_win_rate):
    ...
    if conf_pct >= 90 and (hist_pct is None or hist_pct >= 90):
        color_icon = "🟢 Strong"
    elif conf_pct >= 80 and (hist_pct is None or hist_pct >= 80):
        color_icon = "🟡 Moderate"
    else:
...
def check_breakout(symbol, df, live_price):
    ...
    if squeeze and volume_surge and trend_strong:
        ...
        if (prob and prob >= .7) and (hist_win_rate and hist_win_rate > .70):
...
def check_breakout_imminent(symbol, df, live_price):
    ...
    if latest["bb_width"] < df["bb_width"].rolling(50).mean().iloc[-1] * 0.85 and accumulation:
        ...
        if (prob and prob >= IMMINENT_THRESHOLD) and (hist_win_rate and hist_win_rate > .70):
...
def check_mean_reversion(symbol, df, live_price):
    latest, atr = df.iloc[-1], df["atr"].iloc[-1]
    overbought = latest["rsi"] > 75 and latest["close"] > latest["bb_upper"]
    oversold = latest["rsi"] < 25 and latest["close"] < latest["bb_lower"]
    ...
    if (prob and prob >= 0.7) and (hist_win_rate and hist_win_rate > .70):
...
def run_alerts():
    ...
    for symbol in COINS:
        ...
        df = fetch_data(symbol, granularity='900', limit=8000)
        if df is None or df.empty:
            continue
