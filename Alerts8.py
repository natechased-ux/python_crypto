...
# >>>>>>>>>>>>>>>>>>>>>>>>>  COOLDOWN (added)  <<<<<<<<<<<<<<<<<<<<<<<<<<
COOLDOWN_HOURS = 8
COOLDOWN_STATE_PATH = os.path.join(os.path.dirname(__file__), "cooldown_state.json")

def _load_cooldown_state(path=COOLDOWN_STATE_PATH):
    try:
        with open(path, "r") as f:
            raw = json.load(f)
        # keys are "COIN||SIDE" -> iso timestamp
        return {tuple(k.split("||")): datetime.fromisoformat(v) for k, v in raw.items()}
    except Exception:
        return {}

def _save_cooldown_state(state, path=COOLDOWN_STATE_PATH):
    try:
        ser = {"||".join(k): (v if isinstance(v, str) else v.isoformat()) for k, v in state.items()}
        with open(path, "w") as f:
            json.dump(ser, f)
    except Exception as e:
        print(f"[WARN] Failed to save cooldown state: {e}")

def _can_alert(coin_tag, side, bar_time_utc, cooldown_state, cooldown_hours=COOLDOWN_HOURS):
    if side not in ("LONG", "SHORT"):
        return True
    key = (coin_tag, side)
    last_ts = cooldown_state.get(key)
    if last_ts is None:
        return True
    return (bar_time_utc - last_ts) >= timedelta(hours=cooldown_hours)

def _record_alert(coin_tag, side, bar_time_utc, cooldown_state):
    if side in ("LONG", "SHORT"):
        cooldown_state[(coin_tag, side)] = bar_time_utc
# ======================

# ======================
# Immediate scan (startup)
# ======================
def _immediate_scan(contexts, cooldown_state):
    ...
    for ctx in contexts:
        try:
            ...
            if sig["status"] == "ALERT":
                ...
                # --- cooldown check (startup)
                try:
                    bar_dt = pd.to_datetime(bar_time, utc=True).to_pydatetime()
                except Exception:
                    bar_dt = datetime.now(timezone.utc)
                if _can_alert(ctx.coin_tag, side, bar_dt, cooldown_state, COOLDOWN_HOURS):
                    ...
                    send_telegram(tg)
                    _record_alert(ctx.coin_tag, side, bar_dt, cooldown_state)
                    _save_cooldown_state(cooldown_state)
                else:
                    print(f"[SKIP cooldown] {ctx.coin_sym} {side} @ {bar_time}")
...
def main():
    ...
    # --- load cooldown state (added)
    cooldown_state = _load_cooldown_state()

    # Immediate run + seed
    _startup_times = _immediate_scan(contexts, cooldown_state)
                    ...
                    try:
                        ...
                        if bar_time and last_times.get(ctx.coin_tag) != bar_time:
                            ...
                            if sig["status"] == "ALERT":
                                ...
                                # --- cooldown guard (hour loop)
                                try:
                                    bar_dt = pd.to_datetime(bar_time, utc=True).to_pydatetime()
                                except Exception:
                                    bar_dt = datetime.now(timezone.utc)
                                if not _can_alert(ctx.coin_tag, side, bar_dt, cooldown_state, COOLDOWN_HOURS):
                                    print(f"[SKIP cooldown] {ctx.coin_sym} {side} @ {bar_time}")
                                    continue
