...
def on_message(ws, message):
    ...
    if msg['event'] == 'data':
        ...
        print(f"[{timestamp}] Price: {last_price:.4f}, RSI: {rsi:.2f}, SMA: {sma:.4f}, Imbalance: {imbalance:.2f}%")

        # === BUY / SELL LOGIC ===
        if imbalance > IMBALANCE_BUY and last_price > sma and rsi < 30:
            msg = f"📈 BUY SIGNAL\nPrice: {last_price:.4f}\nRSI: {rsi:.2f}\nImbalance: {imbalance:.2f}%"
            print(msg)
            send_telegram_alert(msg)

        elif imbalance < IMBALANCE_SELL and last_price < sma and rsi > 70:
            msg = f"📉 SELL SIGNAL\nPrice: {last_price:.4f}\nRSI: {rsi:.2f}\nImbalance: {imbalance:.2f}%"
            print(msg)
            send_telegram_alert(msg)
