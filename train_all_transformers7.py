...
OUT_DIR  = "models_transformer_multiscale_safe_stable2"
...
TRADE_MIN, TRADE_MAX = 30, 1000   # <-- enforce this band
...
# --- Epoch loop: select epoch by coarse-threshold val profit WITH trade floor ---
    ...
    # --- Evaluate coarse thresholds and enforce trade band (TRADE_MIN..TRADE_MAX) ---
        best_in_band_p   = -1e9
        best_in_band_n   = 0
        best_in_band_thr = None
        best_in_band_wr  = 0.0

    # Track also the absolute best (even if out of band) for logging only
        abs_best_p, abs_best_n, abs_best_thr, abs_best_wr = -1e9, 0, None, 0.0

        for thr in THRESH_COARSE:
            p, wr, n = grid_eval(maxp, argmax, closes_val, highs_val, lows_val, tp_preds, sl_preds, thr)

        # absolute best (for visibility)
            if p > abs_best_p:
                abs_best_p, abs_best_thr, abs_best_wr, abs_best_n = p, float(thr), wr, n

        # only consider saving if inside the trade band
            if (TRADE_MIN <= n <= TRADE_MAX) and (p > best_in_band_p):
                best_in_band_p   = p
                best_in_band_thr = float(thr)
                best_in_band_wr  = wr
                best_in_band_n   = n

    # --- Logging for this epoch ---
        if best_in_band_thr is not None:
            print(f"{coin} | Epoch {epoch+1}/{EPOCHS} | Val Profit(coarse): {best_in_band_p*100:.4f}% "
                  f"| Trades: {best_in_band_n} | Thr: {best_in_band_thr:.2f}")
        else:
        # No threshold met the minimum trades — show absolute best for debugging, but SKIP saving
            print(f"{coin} | Epoch {epoch+1}/{EPOCHS} | SKIP (trades<{TRADE_MIN}) "
                  f"| Best(any): {abs_best_p*100:.4f}% | Trades: {abs_best_n} | Thr: {abs_best_thr}")

    # --- Early stopping / saving logic ---
        if best_in_band_thr is not None and (best_in_band_p > best_val_profit + 1e-12):
        # Only save if the *in-band* candidate beats prior best
            best_val_profit = best_in_band_p
            best_epoch = epoch + 1
            torch.save(model.state_dict(), best_tmp_path)  # save immediately
            patience = EARLY_STOP_PATIENCE
        else:
            patience -= 1
            if patience <= 0:
