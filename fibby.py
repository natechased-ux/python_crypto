...
class TradeCloser(_th.Thread):
    ...
    def _tick(self):
        ...
        with self.log_lock:
            try:
                df = _pd.read_csv(self.log_path, encoding="utf-8-sig", engine="python", on_bad_lines="skip", quotechar='"')
            except TypeError:
                df = _pd.read_csv(self.log_path, encoding="utf-8-sig", engine="python", error_bad_lines=False, quotechar='"')
...
class OutcomeMonitor(threading.Thread):
    ...
    # 1) file presence + read with UTF-8
        if not os.path.exists(LOG_PATH) or os.path.getsize(LOG_PATH) == 0:
            return
        try:
            df = pd.read_csv(LOG_PATH, encoding="utf-8-sig", engine="python", on_bad_lines="skip", quotechar='"')
        except TypeError:
            df = pd.read_csv(LOG_PATH, encoding="utf-8-sig", engine="python", error_bad_lines=False, quotechar='"')
...
# --- start the outcome monitor once ---
try:
    _OUTCOME_THREAD
except NameError:
    _OUTCOME_THREAD = None
try:
    if _OUTCOME_THREAD is None or not _OUTCOME_THREAD.is_alive():
        _OUTCOME_THREAD = OutcomeMonitor()
        _OUTCOME_THREAD.start()
        print("[Outcome] started")
except Exception as e:
    print("[Outcome] start error:", e)
# --------------------------------------
