...
train_csv = "datasets_macro_training/AAVE_USD.csv"
seq_len = 60
...
def load_model(model_path, scaler_path, input_dim):
    ...
    return model, scalermodel_path, input_dim):
...
def predict_signal(df, model, scaler, feature_order, latest_close):
    ...
    with torch.no_grad():
        ...
        tp_pct = tp_out.item()/100.0
        sl_pct = sl_out.item()/100.0
...
# Force exact training order
                    ...
                    model, scaler = load_model(model_file, input_dim=len(feature_order))
                    ...
                    if first_run or last_signals.get(symbol) != current_signal:
                        last_signals[symbol] = current_signal
                        if strong and label != "HOLD":
                            line = f"🚀 STRONG {label}: {symbol} (p={confidence:.2f}) | TP: {tp_price:.2f} | SL: {sl_price:.2f}"
                        else:
                            line = f"{symbol}: {label} (p={confidence:.2f}) | TP: {tp_price:.2f} | SL: {sl_price:.2f}"
