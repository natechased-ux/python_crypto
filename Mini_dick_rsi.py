...
def passes_volume_spike(candles_5m, percentile=90):
    if len(candles_5m) < 100:
        return False
    vol_threshold = np.percentile(candles_5m["volume"].iloc[-100:], percentile)
    return candles_5m["volume"].iloc[-1] > vol_threshold

def price_above_ema(candles_1h, window=200):
    if len(candles_1h) < window:
        return False
    closes = candles_1h["close"].sort_index()
    ema = closes.ewm(span=window, adjust=False).mean()
    return closes.iloc[-1] > ema.iloc[-1]
...
def process_coin(coin):
    ...
    try:
        ...
        candles_5m = fetch_candles(coin, CANDLE_GRANULARITY)
        candles_5m = calculate_stoch_rsi(candles_5m)

        # Get 1H candles for EMA check
        candles_1h = fetch_candles(coin, 3600)

        # === Apply filters ===
        if not passes_volume_spike(candles_5m):
            print(f"{coin.upper()} skipped — no volume spike")
            return
        if not price_above_ema(candles_1h) and not any(c["price"].median() < current_price for c in book["bids"]):
            print(f"{coin.upper()} skipped — price below 200 EMA (no SHORT)")
            return
        if price_above_ema(candles_1h) and not any(c["price"].median() > current_price for c in book["asks"]):
            print(f"{coin.upper()} skipped — price above 200 EMA (no LONG)")
            return
        ...
        for cluster in asks:
            ...
            if cluster_price > current_price and CLUSTER_MIN_DISTANCE <= distance <= CLUSTER_MAX_DISTANCE and k_now > k_prev and k_now > 20:
                tp = cluster_price
                sl = current_price - SL_BUFFER_PCT * current_price
                message = (
                    f"🚀 LONG (toward ask cluster) for {coin.upper()}\n"
                    f"Price: {format_price(current_price)}\n"
                    f"Target cluster @ {format_price(tp)}\n"
                    f"TP: {format_price(tp)}\nSL: {format_price(sl)}\n"
                    f"Stoch RSI rising (k: {k_now:.2f})"
                )
        ...
        for cluster in bids:
            cluster_price = cluster["price"].median()
            distance = abs(current_price - cluster_price) / current_price * 100
            if cluster_price < current_price and CLUSTER_MIN_DISTANCE <= distance <= CLUSTER_MAX_DISTANCE and k_now < k_prev and k_now < 80:
                tp = cluster_price
                sl = current_price + SL_BUFFER_PCT * current_price
                message = (
                    f"🔻 SHORT (toward bid cluster) for {coin.upper()}\n"
                    f"Price: {format_price(current_price)}\n"
                    f"Target cluster @ {format_price(tp)}\n"
                    f"TP: {format_price(tp)}\nSL: {format_price(sl)}\n"
                    f"Stoch RSI falling (k: {k_now:.2f})"
                )
                send_telegram_message(message)
                last_alert_time[coin] = now
                return
