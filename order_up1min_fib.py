...
def analyze_order_book(symbol, data):
    try:
        ...
        if long_signal:
            ...
            return (f"fib_1min{symbol.upper()}: LONG Signal | Entry: {format_price(close_price)}, TP: {format_price(tp)}, SL: {format_price(sl)}\n"
            ...
            return (f"fib_1min{symbol.upper()}: SHORT Signal | Entry: {format_price(close_price)}, TP: {format_price(tp)}, SL: {format_price(sl)}\n"
