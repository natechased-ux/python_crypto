...
# -*- coding: utf-8 -*-
"""
obflow_alerts_rules_hotreload_earlycut.py
-----------------------------------------
- Per-symbol tightened-rule alerts (hot-reload JSON)
- Telegram alerts include p_win and EV, dynamic price formatting
- Early Cut module:
    * SHORTS: check at +15m; cut if still adverse beyond buffer
    * LONGS:  check at +30m; cut if still adverse beyond buffer
"""
...
# --------- Setups JSON path (Windows preferred, hard-coded) ---------
...
def _compute_return_pct(side: str, entry: float, current: float) -> float:
    # Positive means favorable in the direction of the trade
    if side == "long":
...
def _schedule_early_cut(symbol: str, side: str, entry_price: float) -> None:
    ...
    if side == "long":
        after = EARLY_CUT_LONG_SEC; buf = EARLY_CUT_LONG_BUFFER_PCT
    else:
        after = EARLY_CUT_SHORT_SEC; buf = EARLY_CUT_SHORT_BUFFER_PCT
    _PENDING_EARLY_CUTS.append({
        "symbol": symbol, "side": side, "entry": float(entry_price),
        "ts": time.time(), "cut_after_sec": int(after), "buffer_pct": float(buf)
    })
...
def _process_early_cuts() -> None:
    ...
    for item in list(_PENDING_EARLY_CUTS):
        ...
        try:
            ...
            adverse = (ret <= -buf)  # adverse if return below -buffer
            if adverse:
...
def _level_flags_and_stoch(sym: str, price: float) -> Dict[str, Any]:
    ...
    def _near_fib_flags(df_lohi: pd.DataFrame, lo_bars: int, hi_bars: int, key: str):
        ...
        out[f"near_{key}_min"] = _price_near_level(price, zmin); out[f"near_{key}_max"] = _price_near_level(price, zmax)
        out[f"in_{key}_zone"] = int(zmin <= price <= zmax)
    if not df1h.empty:
        _near_fib_flags(df1h, 2*24, 2*24, "fib_tiny"); _near_fib_flags(df1h, 7*24, 7*24, "fib_short")
    df6h = fetch_candles(sym, 21600)
    if not df6h.empty:
        _near_fib_flags(df6h, 14*4, 14*4, "fib_medium"); _near_fib_flags(df6h, 30*4, 30*4, "fib_long")
    # stoch
    ...
    out["stoch_k_1h"], out["stoch_d_1h"] = k1h, d1h; out["stoch_k_15m"], out["stoch_d_15m"] = k15, d15
    return out
