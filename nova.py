...
IMBALANCE_BUY = 60
IMBALANCE_SELL = 40
PRICE_CHANGE_THRESHOLD = 1.0  # Percentage threshold for significant trend
...
def on_message(ws, message):
    ...
    if msg['event'] == 'data':
        ...
        print(f"[{datetime.utcnow()}] Price: {last_price:.4f}, RSI: {rsi:.2f}, SMA: {sma:.4f}, Change: {price_change:.2f}%, Imbalance: {imbalance:.2f}%")

        # === TREND REVERSAL + ORDER BOOK LOGIC ===
        if price_change > PRICE_CHANGE_THRESHOLD:
            # Uptrend reversal logic
            if imbalance > IMBALANCE_BUY and rsi < 30 and last_price > sma:
                msg = (f"📈 STRONG BUY SIGNAL\n"
                       f"Price: {last_price:.4f}\nRSI: {rsi:.2f}\nSMA: {sma:.4f}\n"
                       f"Imbalance: {imbalance:.2f}%\nChange: {price_change:.2f}%")
                print(msg)
                send_telegram_alert(msg)

            # Downtrend reversal logic
            elif imbalance < IMBALANCE_SELL and rsi > 70 and last_price < sma:
                msg = (f"📉 STRONG SELL SIGNAL\n"
                       f"Price: {last_price:.4f}\nRSI: {rsi:.2f}\nSMA: {sma:.4f}\n"
                       f"Imbalance: {imbalance:.2f}%\nChange: {price_change:.2f}%")
                print(msg)
                send_telegram_alert(msg)
