...
LOOKAHEAD_HOURS = 8  # for 1-hour candles, that's 6 future candles

import numpy as np

import numpy as np

def create_labels(df, future_steps=8, threshold=0.0225):
    """
    Create trading labels (HOLD=0, BUY=1, SELL=2) shifted forward to prevent look-ahead bias.
    future_steps: number of steps ahead to check for price change
    threshold: minimum % move to consider a trade signal
    """
    closes = df["close"].values
    labels = np.zeros(len(df))  # HOLD by default
    ...
    # Shift labels forward so the model only sees past info
    df["label"] = pd.Series(labels).shift(-future_steps)
...
def generate_tp_sl_labels(df, lookahead=8, atr_mult_tp = 2.75, atr_mult_sl=1.4):
    """
    Generate TP/SL percentages based only on *future data* shifted forward.
    """
    ...
    tp_pct_list = [np.nan] * len(df)
    sl_pct_list = [np.nan] * len(df)

    for i in range(len(df) - lookahead):
        entry_price = closes[i]
        tp_level = entry_price * (1 + atr_mult_tp * atr[i] / entry_price)
        sl_level = entry_price * (1 - atr_mult_sl * atr[i] / entry_price)
        ...
        for j in range(1, lookahead + 1):
            if highs[i + j] >= tp_level:
                tp_hit = True
                tp_pct_list[i] = (tp_level - entry_price) / entry_price
                break
            if lows[i + j] <= sl_level:
                sl_hit = True
                sl_pct_list[i] = (entry_price - sl_level) / entry_price
                break

        # If neither hit, default to ATR-based estimate
        if not tp_hit:
            tp_pct_list[i] = (atr_mult_tp * atr[i]) / entry_price
        if not sl_hit:
            sl_pct_list[i] = (atr_mult_sl * atr[i]) / entry_price

    # Shift TP/SL forward to align with shifted label
    df["tp_pct"] = pd.Series(tp_pct_list).shift(-lookahead)
    df["sl_pct"] = pd.Series(sl_pct_list).shift(-lookahead)
...
def main():
    ...
    for coin in COINS:
        ...
        df = create_labels(df, future_steps=future_steps, threshold=0.0225)
        df = generate_tp_sl_labels(df, lookahead=future_steps, atr_mult_tp=2.75, atr_mult_sl=1.4)
