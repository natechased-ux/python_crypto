...
OUT_DIR  = "models_transformer_multiscale_safe_stable3"
...
# Threshold search
THRESH_COARSE = [0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85]  # widened grid
FINE_RADIUS   = 0.08   # +/- range around coarse winner
FINE_STEP     = 0.01   # 0.01 granularity for fine search
THR_MIN, THR_MAX = 0.50, 0.95

# Reference threshold used ONLY for model selection (stable early-stopping)
REF_THR_FOR_ESTOP = 0.65

# Stability guards for final threshold selection
TRADE_MIN, TRADE_MAX = 20, 1000   # <-- enforce this band
MIN_PROFIT_DELTA = 0.001        # +0.10% absolute improvement required to adopt refined threshold
...
def train_coin(coin_file):
    ...
    best_tmp_path = os.path.join(OUT_DIR, f"{coin}_BEST_TMP.pth")
    if os.path.exists(best_tmp_path):
        try: os.remove(best_tmp_path)
        except Exception: pass

    # --- Epoch loop: select epoch by coarse-threshold val profit WITH trade floor ---
# --- Epoch loop: select epoch by coarse-threshold val profit WITH trade floor ---
    # --- Epoch loop: train weights; select epoch by a stable metric (no threshold surfing) ---
    best_sel_metric = float('inf')   # lower is better
    best_epoch = -1
    patience = EARLY_STOP_PATIENCE
    best_tmp_path = os.path.join(OUT_DIR, f"{coin}_BEST_TMP.pth")
    if os.path.exists(best_tmp_path):
    ...
    # ---- validation (collect losses + predictions once) ----
        model.eval()
        val_losses = []
        probs_list, tp_list, sl_list = [], [], []
        with torch.no_grad():
            for xb, yb_cls, yb_tp, yb_sl in val_loader:
                xb = xb.to(DEVICE)
                lc, tp, sl = model(xb)
            # loss for stable selection
                vloss = (loss_cls(lc, yb_cls.to(DEVICE)).item()
                         + loss_reg(tp, yb_tp.to(DEVICE)).item()
                         + loss_reg(sl, yb_sl.to(DEVICE)).item())
                val_losses.append(vloss)
            # predictions for fixed-threshold profit (for selection visibility)
                probs_list.append(torch.softmax(lc, dim=1).cpu().numpy())
                tp_list.append(tp.squeeze().cpu().numpy())
                sl_list.append(sl.squeeze().cpu().numpy())

        probs = np.concatenate(probs_list, axis=0)
        tp_pred = np.concatenate(tp_list, axis=0)
        sl_pred = np.concatenate(sl_list, axis=0)
        maxp_np = probs.max(axis=1)
        arg_np  = probs.argmax(axis=1)

    # grid_eval expects torch tensors for mask/clone logic
        maxp_t = torch.tensor(maxp_np)
        arg_t  = torch.tensor(arg_np)

    # Profit at a FIXED reference threshold (stable across epochs)
        ref_p, ref_wr, ref_n = grid_eval(maxp_t, arg_t, closes_val, highs_val, lows_val, tp_pred, sl_pred, REF_THR_FOR_ESTOP)

    # Selection metric: prefer higher ref profit; break ties with mean val loss
    # (lower is better, hence the negative sign on ref_p)
        sel_metric = -ref_p + 0.01 * float(np.mean(val_losses))

        print(f"{coin} | Epoch {epoch+1}/{EPOCHS} | val_loss={float(np.mean(val_losses)):.5f} "
              f"| ref_thr={REF_THR_FOR_ESTOP:.2f} | ref_profit={ref_p*100:.3f}% | ref_trades={ref_n}")

        if sel_metric < best_sel_metric - 1e-12:
...
# --- Final thresholding on best epoch ---
    ...
    # --- Final thresholding on best epoch ---
    if os.path.exists(best_tmp_path):
        model.load_state_dict(torch.load(best_tmp_path, map_location='cpu'), strict=True)
        print(f"{coin} 🔒 Reloaded best epoch: {best_epoch} (Val Profit≈ {best_val_profit*100:.4f}%)")
    else:
        print(f"{coin} ⚠️ No best checkpoint found; using last state")
    ...
    # 1) Coarse winner — choose best INSIDE trade band if possible
    ...
    # Re-evaluate within trade band
    best_in_band = (-1e9, None, 0.0, 0)
    for thr in THRESH_COARSE:
        p, wr, n = grid_eval(maxp, argmax, closes_val, highs_val, lows_val, tp_preds, sl_preds, thr)
        if TRADE_MIN <= n <= TRADE_MAX and p > best_in_band[0]:
            best_in_band = (p, float(thr), wr, n)
    if best_in_band[1] is not None:
        coarse_best_p, coarse_thr, coarse_wr, coarse_n = best_in_band

    # 2) Fine search around coarse winner (already enforces band)
    ...
    # 3) Stability guard: adopt refined if it clearly improves OR ties with better trade count
    EPS = 1e-6
    final_thr = coarse_thr
    final_p, final_wr, final_n = coarse_best_p, coarse_wr, coarse_n

    better_profit = (refined_p >= coarse_best_p + MIN_PROFIT_DELTA)
    tie_profit    = (abs(refined_p - coarse_best_p) <= EPS)

    if (TRADE_MIN <= refined_n <= TRADE_MAX) and (better_profit or (tie_profit and refined_n > coarse_n)):
