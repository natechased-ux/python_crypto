...
symbols = ["btc-usd", "eth-usd", "xrp-usd", "ltc-usd", "ada-usd",
           ...
           "axs-usd"]  # Add your symbols here
granularity = 900  # 6 hours in seconds
atr_length = 14
hold_bars = 6  # Optional hold period info for alerts
sl_atr_mult = 1.0
tp_atr_mult = 2.0
...
async def send_alert(symbol, idx, div_type, data):
    ...
    msg = (
        f"📈 {symbol.upper()} 15 min CVD Divergence Alert!\n"
...
async def monitor():
    last_alerts = {}  # (symbol, div_type) -> last_alert_time (unix timestamp)

    # Initialize last_alerts with latest candle times on startup (prevents old alerts)
    for symbol in symbols:
        data = fetch_historical_data(symbol)
        latest_time = data['time'].max()
        last_alerts[(symbol, 'bullish')] = latest_time
        last_alerts[(symbol, 'bearish')] = latest_time
        ...
        try:
            for symbol in symbols:
                ...
                for idx, div_type in divergences:
                    alert_time = data.loc[idx, 'time']
                    key = (symbol, div_type)
                    last_alert_time = last_alerts.get(key, 0)
                    if alert_time > last_alert_time:
                        await send_alert(symbol, idx, div_type, data)
                        last_alerts[key] = alert_time
