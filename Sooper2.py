...
Refactor highlights
-------------------
- Mean‑reversion gets per‑coin thresholds (JSON) + optional auto‑tuner from alert CSV
...
def train_model(label_func, save_path: Path, label_name: str, historical_data: dict[str, pd.DataFrame]) -> bool:
    ...
    if label_name == "MeanRev":
        ...
        if pos_count == 0:
            print("❌ MeanRev has zero positives; relax label or increase history.")
            return False
...
def main_loop():
    print("🚀 Starting Refactored Multi‑Strategy Bot (coin‑aware)…")
    retrain_if_needed()
    while True:
        t0 = time.time()
        run_once()
