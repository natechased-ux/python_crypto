...
symbols = ["btc-usd", "eth-usd", "xrp-usd", "ltc-usd", "ada-usd",
           ...
           "axs-usd"]
granularity = 21600  # 1 hour in seconds
atr_length = 14
hold_bars = 6
sl_atr_mult = 1.0
tp_atr_mult = 2.0
...
# Rate-limited fetch
async def rate_limited_fetch(symbol):
    for _ in range(3):
        try:
...
async def send_alert(symbol, idx, div_type, data):
    ...
    msg = (
        f"📈 {symbol.upper()} 6 hr CVD Divergence Alert!\n"
        ...
        f"Hold period: approx {hold_bars * (granularity // 21600)}h"
    )
...
async def monitor():
    ...
    for symbol in symbols:
        data = await rate_limited_fetch(symbol)
        if data is not None:
            last_alerts[(symbol, 'bullish')] = len(data) - 1
            last_alerts[(symbol, 'bearish')] = len(data) - 1
        await asyncio.sleep(0.5)  # avoid burst requests
    ...
    while True:
        try:
            for symbol in symbols:
                ...
                await asyncio.sleep(0.5)  # throttle between coins

            print(f"⏰ 6 hr DIV — {pd.Timestamp.now()}")
