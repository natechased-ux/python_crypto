...
min_distance_pct = 0.0001  # 0.01% min distance from price for whale orders
entry_distance_pct = 0.0015  # 0.15% distance for entry signal near whale level
...
def get_dynamic_whale_threshold(order_book):
    ...
    whale_threshold = average_size * 100  # more strict
...
def analyze_order_book(symbol, data):
    try:
        ...
        long_signal = nearest_bid and abs(close_price - nearest_bid[0]) / nearest_bid[0] <= entry_distance_pct and close_price > 0.98 * vwap
        short_signal = nearest_ask and abs(close_price - nearest_ask[0]) / nearest_ask[0] <= entry_distance_pct and close_price < 1.02 * vwap
        ...
        if long_signal:
            ...
            sl2 = close_price - atr
            return (f"\n📈 1hr A {symbol.upper()}: LONG Signal\nTime: {pst_now}\n"
            ...
            sl1 = close_price + (atr * 0.5)
            sl2 = close_price + atr
            return (f"\n📉 1hr A {symbol.upper()}: SHORT Signal\nTime: {pst_now}\n"
                    f"Entry: {format_price(close_price)}, TP1: {format_price(tp1)}, TP2: {format_price(tp2)}\n"
                    f"SL1: {format_price(sl1)}, SL2: {format_price(sl2)}\n"
