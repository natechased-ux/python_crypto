...
SAVE_PATH = "datasets_macro_training4"
...
# --- Patched TP/SL label generation ---
def generate_tp_sl_labels(df, lookahead=LOOKAHEAD_HOURS, atr_mult_tp=ATR_TP, atr_mult_sl=ATR_SL):
    highs = df['high'].values
    lows = df['low'].values
    closes = df['close'].values
    ...
    tp_pct_list = [0.0] * len(df)
    sl_pct_list = [0.0] * len(df)

    for i in range(len(df) - lookahead):
        entry_price = closes[i]
        future_high = max(highs[i+1:i+lookahead+1])
        future_low = min(lows[i+1:i+lookahead+1])

        tp_pct = max(0, (future_high - entry_price) / entry_price)
        sl_pct = max(0, (entry_price - future_low) / entry_price)

        # ATR fallback if no movement
        if tp_pct == 0:
            tp_pct = (atr_mult_tp * atr[i]) / entry_price
        if sl_pct == 0:
            sl_pct = (atr_mult_sl * atr[i]) / entry_price

        tp_pct_list[i] = tp_pct
        sl_pct_list[i] = sl_pct
