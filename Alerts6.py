...
# cache for one BTC fetch per scan
_LIVE_BTC = {"cut": None, "df": None}

def get_btc_live_for_scan(granularity=GRANULARITY):
    """Return BTC DataFrame aligned to the current top-of-hour (cached)."""
    cut = datetime.now(timezone.utc).replace(minute=0, second=0, microsecond=0)
    if _LIVE_BTC["cut"] != cut or _LIVE_BTC["df"] is None:
        ...
        # add time features
        btc["hour"] = btc["time"].dt.hour / 23.0
        btc["day_of_week"] = btc["time"].dt.dayofweek / 6.0
        btc["day_of_month"] = btc["time"].dt.day / 31.0

        # prefix cols except time
...
def merge_btc(df):
    """Merge the cached live BTC data into this coin's DataFrame."""
    btc_df = get_btc_live_for_scan()
    if btc_df is None or btc_df.empty:
        return df

    # Drop cheating label/TP/SL columns if they exist
    drop_cols = [c for c in btc_df.columns if c in ("btc_label", "btc_tp_pct", "btc_sl_pct")]
    if drop_cols:
        print(f"[INFO] Dropping BTC columns not used in live: {drop_cols}")
...
# ---- Fallback live build (no btc_label/tp/sl, but at least core btc_* exists) ----
    start = end - timedelta(days=60)
    btc_live = fetch_range("BTC-USD", start, end, granularity)
    if btc_live is None or btc_live.empty:
        print(f"[WARN] BTC live fetch empty; skipping BTC merge")
        return df
    ...
    btc_live = btc_live.rename(columns={c: (c if c == "time" else f"btc_{c}") for c in btc_live.columns})
    btc_live = btc_live[[c for c in btc_live.columns if c == "time" or c.startswith("btc_")]]

    # Drop cheating label/TP/SL columns from fallback too
    drop_cols = [c for c in btc_live.columns if c in ("btc_label", "btc_tp_pct", "btc_sl_pct")]
    if drop_cols:
        print(f"[INFO] Dropping BTC columns not used in live: {drop_cols}")
        btc_live = btc_live.drop(columns=drop_cols)
...
def fix_merge_suffixes(df: pd.DataFrame) -> pd.DataFrame:
    #"\"\"Resolve *_x/*_y created by merges; keep base for coin, drop duplicates.\"\"\"
    for col in ["open", "high", "low", "close", "volume", "hour", "day_of_week", "day_of_month"]:

        x = f"{col}_x"
        y = f"{col}_y"
        if x in df.columns and col not in df.columns:
            df = df.rename(columns={x: col})
        if y in df.columns:
...
def strip_merge_suffixes(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize columns by removing *_x and dropping *_y created by merges."""
    rename = {}
    drop = []
...
# Map last window back to the original (unscaled) row for time & entry price
        last_idx = int(y_idx[-1])
        row_idx  = df_num.index[last_idx]               # index in the original df space
        bar_time = df.loc[row_idx, "time"]
        entry_px = float(df.loc[row_idx, "close"])

# Return the batch tensor and a small dict with what infer() needs
        return X[-1:].float(), {"time": bar_time, "entry": entry_px}
    ...
    def infer(self):
        ...
        # Softmax -> probabilities for [HOLD, LONG, SHORT]
            ...
            return {
                ...
                "tp_pct": tp_pct,
                "sl_pct": sl_pct,
                "entry": entry_price,          # <- use 'entry' everywhere downstream
                "prob_hold": p_hold,
                "prob_long": p_long,
                "prob_short": p_short,
            }
...
def _immediate_scan(contexts):
    ...
    results = {}  # coin_tag -> bar_time we acted on
    for ctx in contexts:
        try:
            sig = ctx.infer()  # returns {"time": ..., "entry": ..., ...}
            ...
            if sig["status"] == "ALERT":
                ...
                entry = float(sig.get("entry", float("nan")))  # <- raw close of last closed bar
                ...
                # Telegram (enabled for immediate run)
                tg_text = (
                    f"{ctx.coin_sym} {side}\n"
                    f"Time: {bar_time}\n"
...
def main():
    ...
    # Immediate run once on startup (latest closed candle)
        # Immediate run once on startup (latest closed candle) and seed last_times
