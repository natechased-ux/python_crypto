#!/usr/bin/env python3
"""
TTM Squeeze (LazyBear updated) • 15m • Pro Triggers
...
# ==== Pro trigger gates ====
SCORE_MIN = 75              # require B-tier or better
BBW_LOOKBACK = 200
BBW_PCTL_MAX = 25           # 30th percentile or tighter
ADX_SLOPE_LOOKBACK = 3
ADX_SLOPE_MIN = 1.5         # ADX - ADX[3] >= 2.0
DI_SPREAD_MIN = 7
MIN_SQZ_BARS = 8
ALLOW_FADING_IF_SCORE_GE = 88

# HEADS-UP settings
HEADSUP_ADX_RISING = True
HEADSUP_MIN_SQZ = 5
HEADSUP_MSG_COOLDOWN_MIN = 45

# Misc
COOLDOWN_MIN = 12
RATE_LIMIT_PAUSE = 0.8
SAVE_STATE_EACH_LOOP = True
STATE_PATH = "positions_state_15m_pro.json"
DEBUG = False
    ...
    # Final gate: need (ADX rising OR very strong momentum), plus the quality checks
        ...
        if entry_signal and (pos is None or not pos.is_active):
            ...
            if score_pct >= SCORE_MIN and momentum_preference_ok(score_pct):
                ...
                header = f"{badge} <b>ENTRY • TTM Squeeze 15m (pro)</b> {badge}"
                txt = (
                    f"{header}\n"
                    f"<b>{sym}</b> • {direction}\n"
...
def main():
    ...
    print("Starting TTM Squeeze 15M (pro) Telegram Alert...")
    while True:
        try:
            run_once(state)
