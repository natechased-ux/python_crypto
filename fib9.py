    ...
    while True:
        ...
        for coin in COINS:
            try:
                ...
                # 'alert' is now a dict from check_signal; keep backward compatibility if it's a string
                if isinstance(alert, dict):
                    info = alert
                    now = datetime.now(timezone.utc)
                    # Cooldown check (prevent spam): only enforce when we plan to send
                    last = cooldowns.get(coin)
                    # Evaluate C1–C3 combos
                    zoneU = str(info.get("zone","" )).upper()
                    sideU = str(info.get("side","" )).upper()
                    symU  = str(info.get("symbol","" )).upper()
                    sells = float(info.get("sells_usd", 0.0))

                    C1 = (sells <= 143.0) and (zoneU == "EMA50_1D")
                    C2 = (sideU == "LONG") and (zoneU == "EMA50_1D")
                    C3 = (sells <= 143.0) and (symU in {"UNI/USD","HBAR/USD"})

                    combos_ok = (C1 or C2 or C3)

                    if combos_ok:
                        # Enforce cooldown only for actual alerts
                        if last and (now - last).total_seconds() < COOLDOWN_MINUTES*60:
                            pass  # still cooling down; we already logged in check_signal
                        else:
                            send_telegram(info["msg"])
                            cooldowns[coin] = now
                            # Update last_alert_key/state for dedupe
                            STATE["last_alert_ts"][coin] = now.timestamp()
                            STATE["last_alert_key"][coin] = info.get("alert_key", "")
                            save_state(STATE)
                    # else: log only (already done inside check_signal)
                else:
                    # Backward-compatibility: if a plain string comes back, do not Stoch-gate; do not score-gate.
                    # We do NOT send in this case since new policy is combos-only alerts; log already happened.
                    pass
            except Exception as e:
