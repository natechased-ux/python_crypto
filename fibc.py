...
def format_simple_alert(symbol: str, entry: float, tp: float, sl: float) -> str:
    ...
    return f"{sym} | Entry: ${_fmt_dyn(entry)} | TP: ${_fmt_dyn(tp)} | SL: ${_fmt_dyn(sl)}"

# Prefer TP2 > TP1 > TP (single TP in output)
_RX_TP2 = _re.compile(r"(?is)(?P<sym>[A-Z0-9/\-]+).*?Entry[: ]*\$?(?P<entry>[-0-9\.]+).*?TP2[: ]*\$?(?P<tp>[-0-9\.]+).*?SL[: ]*\$?(?P<sl>[-0-9\.]+)")
_RX_TP1 = _re.compile(r"(?is)(?P<sym>[A-Z0-9/\-]+).*?Entry[: ]*\$?(?P<entry>[-0-9\.]+).*?TP1[: ]*\$?(?P<tp>[-0-9\.]+).*?SL[: ]*\$?(?P<sl>[-0-9\.]+)")
_RX_TP  = _re.compile(r"(?is)(?P<sym>[A-Z0-9/\-]+).*?Entry[: ]*\$?(?P<entry>[-0-9\.]+).*?TP[: ]*\$?(?P<tp>[-0-9\.]+).*?SL[: ]*\$?(?P<sl>[-0-9\.]+)")

def _send_telegram_raw(msg: str):
    for name in ("send_telegram_api", "telegram_send", "tg_send", "_orig_send_telegram_message"):
        fn = globals().get(name)
        if callable(fn):
            return fn(msg)
    try:
...
def send_telegram_message(msg: str):
    try:
        m = _RX_TP2.search(msg or "") or _RX_TP1.search(msg or "") or _RX_TP.search(msg or "")
        if m:
            sym   = m.group("sym").replace("-", "/").upper()
            entry = float(m.group("entry"))
            tp    = float(m.group("tp"))
            sl    = float(m.group("sl"))
            simple = format_simple_alert(sym, entry, tp, sl)
            return _send_telegram_raw(simple)
    except Exception:
        pass
    return _send_telegram_raw(msg)

def send_alert(symbol, entry, tp, sl):
    try:
...
if __name__ == "__main__":
    ...
    try:
        ...
        pass
    try:
        # If the script defines OutcomeMonitor, start it so TP/SL + MAE/MFE outcomes are filled.
    ...
    try:
        ...
        # If main_loop isn't defined here, ignore.
        pass
