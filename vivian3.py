...
PRODUCTS = ["SUI-USD", "SEI-USD", "XRP-USD", "crv-usd","ltc-usd","ada-usd","doge-usd",
    "sol-usd","wif-usd","ondo-usd", "avax-usd","xcn-usd","uni-usd","mkr-usd"]  # add/remove freely

ONLY_ALERT_ON_VIABLE = True          # send Telegram ONLY when a trade plan passes all filters
SEND_NONVIABLE_SUMMARY = False       # if False, non-viable bursts are silent

# --- Core aggression (size/speed) ---
LARGE_TRADE_PERCENTILE = 95         # 95→looser, 98–99→stricter
BURST_COUNT = 4                      # require >= N large prints
BURST_WINDOW_MS = 1000                # within Y milliseconds

# --- Notional floors ---
MIN_SINGLE_TRADE_NOTIONAL_USD = 0   # set 0 to disable; majors: 150k–300k
MIN_BURST_NOTIONAL_USD = 100_000         # total notional of large prints in burst

# --- Directional dominance ---
MIN_DIRECTIONAL_DOMINANCE = 0.70     # ≥75% prints same direction
MIN_SIGNED_NOTIONAL_USD = 100_000    # same-direction notional floor (0 to disable)

# --- Displacement & sweep ---
MIN_PRICE_DISPLACEMENT_PCT = 0.10 / 100   # must move ≥0.25% during burst window
MIN_DISTINCT_LEVELS = 2                   # must sweep ≥3 distinct price levels

# --- Bigger-swing trade constraints ---
MIN_RR = 1.4                          # minimum acceptable R:R
USE_ATR = True                        # use ATR floors to avoid micro moves
ATR_LEN = 120                         # samples for pseudo-ATR from last trade prices
MIN_RISK_ATR = 0.18                   # SL distance ≥ 0.18 * ATR (abs)
MIN_TP_ATR   = 0.36                   # TP distance ≥ 0.36 * ATR (abs)

# --- Cluster logic (adaptive by ATR%) ---
BASE_CLUSTER_BIN_PCT = 0.0025         # ~0.25% of price; scaled by asset_profile()
CLUSTER_TOP_PERCENTILE = 95           # consider only top ≥ percentile clusters
CLUSTER_NEARBY_PCT = 0.02            # cluster must be within 0.8% of price to be "nearby"
SL_BUFFER_PCT = 0.0025                # push SL ~0.15% beyond anchor cluster

# --- TP distance gate (absolute % floor; adaptive) ---
BASE_MIN_TP_PCT = 0.006               # base 0.8% min TP distance; scaled by ATR%
...
# trade_buffer entries: (ts, price, size_base, signed_size, notional)
trade_buffer = {p: collections.deque() for p in PRODUCTS}
large_trade_times = {p: collections.deque() for p in PRODUCTS}
burst_notional = {p: collections.deque() for p in PRODUCTS}
...
def asset_profile(product):
    """
    Scale parameters by current ATR%:
      - Low vol (atr% < 0.4%): tighter bins, smaller TP floor, easier burst
...
def prune_old_trades(product):
    cutoff = now_ts() - 600  # 10 min rolling window
...
def percentile_threshold(product):
    ...
    sizes = [abs(sz) for (_, _, sz, _, _) in trade_buffer[product]]
    if len(sizes) < 20:
        return None
...
def propose_trade_plan(product, flow_bias, price, near_bid, near_ask):
    ...
    if flow_bias == "Aggressive BUY" and near_bid:
        ...
        if c_px <= price:
            ...
            # TP cluster with minimum distance
            ...
            if tp_info:
                ...
                tp = entry + max(MIN_RR * risk, (MIN_TP_ATR * (get_atr_abs(product) or 0.0)) if USE_ATR else 0.0)
...
def handle_market_trades(product, trades):
    ...
    for t in trades:
        ...
        # per-trade notional floor for counting as "large"
        is_large = (th is not None and size >= th)
        if MIN_SINGLE_TRADE_NOTIONAL_USD > 0 and notional < MIN_SINGLE_TRADE_NOTIONAL_USD:
            is_large = False
    ...
    # Burst gates: count + total notional (adaptive)
    prof = asset_profile(product)
    needed_n = prof["BURST_N"]
    total_notional = sum(list(burst_vals))
    notional_ok = total_notional >= max(MIN_BURST_NOTIONAL_USD, prof["MIN_BURST_NOTIONAL"])

    # If burst insufficient or cooldown active, stop early
    if not (len(burst_times) >= needed_n and notional_ok and (ts_now - last_alert_time[product]) > ALERT_COOLDOWN_SEC):
        return

    entry_px = last_price[product]

    # Directional dominance within the burst window
    recent = [rec for rec in trade_buffer[product] if (ts_now - rec[0]) * 1000 <= BURST_WINDOW_MS]
    dir_flags = []    # +1 for Aggressive BUY, -1 for Aggressive SELL
    signed_notionals = []
    for (tss, px, sz, sgn, nto) in recent:
        if sgn > 0:
            dir_flags.append(1); signed_notionals.append(nto)
        elif sgn < 0:
            dir_flags.append(-1); signed_notionals.append(-nto)

    dominance = (abs(sum(dir_flags)) / len(dir_flags)) if dir_flags else 0.0
    if dominance < MIN_DIRECTIONAL_DOMINANCE:
        return

    if MIN_SIGNED_NOTIONAL_USD > 0 and abs(sum(signed_notionals)) < MIN_SIGNED_NOTIONAL_USD:
        return

    # Displacement during the burst
    bw_start = burst_times[0] if burst_times else ts_now
    bw_end = burst_times[-1] if burst_times else ts_now
    window_prices = [px for (ts, px, *_ ) in trade_buffer[product] if bw_start <= ts <= bw_end]
    if len(window_prices) >= 2:
        disp = abs(window_prices[-1] - window_prices[0]) / max(1e-12, window_prices[0])
        if disp < MIN_PRICE_DISPLACEMENT_PCT:
            return

    # Sweep test: distinct levels traded
    if MIN_DISTINCT_LEVELS > 1 and window_prices:
        levels = set(round(p, dyn_decimals(entry_px)) for p in window_prices)
        if len(levels) < MIN_DISTINCT_LEVELS:
            return

    # Determine flow bias from last N large prints (approx via recent signs)
    N = needed_n
    recent_signs = [sg for (_, _, _, sg, _) in list(trade_buffer[product])[-max(N*3, N+5):]]
    flow_bias = "Aggressive BUY" if sum(recent_signs[-N:]) > 0 else "Aggressive SELL"
    ...
    # If only viable trades should alert and no plan, bail silently
    if ONLY_ALERT_ON_VIABLE and not plan:
        last_alert_time[product] = ts_now
        large_trade_times[product].clear()
        burst_notional[product].clear()
        return
    ...
    msg = [
        ...
        f"Burst: <b>{len(large_trade_times[product])}</b> prints in ≤ {BURST_WINDOW_MS} ms (notional ≈ ${total_notional:,.0f})",
        f"ATR% (est): <b>{(prof['ATR_PCT']*100):.2f}%</b>",
        f"Flow bias: <b>{flow_bias}</b> (dominance ~{dominance*100:.0f}%)"
    ]
    ...
    if plan:
        side = plan["side"]; rr = plan["rr"]; entry = plan["entry"]; sl = plan["sl"]; tp = plan["tp"]
        ...
        if not ONLY_ALERT_ON_VIABLE and SEND_NONVIABLE_SUMMARY:
            msg += ["", "Skipped: aggression/tradeability filters not met."]
        else:
            last_alert_time[product] = ts_now
            large_trade_times[product].clear()
            burst_notional[product].clear()
...
def handle_level2(product, updates):
    ...
    # gentle decay to let old clusters fade
    for s in ("bid", "ask"):
...
def run_ws():
    while True:
        try:
            ...
            print("WS exception, reconnecting in 3s:", e)
            time.sleep(3)


if __name__ == "__main__":
    print("Starting Vivian-style Coinbase watcher v3.1 (strict)…")
