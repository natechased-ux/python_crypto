# session_cycle_backtest_filtered_v2.py
"""
Enhanced filtered backtester with a walk-forward edge threshold.

New:
- --edge_threshold <float> (in percent units, e.g., 0.002 = 0.2%)
  Only take a trade if |mean_ret_lookback| >= edge_threshold and the direction follows the sign.
- Everything else is identical to session_cycle_backtest_filtered.py

Usage example:
py session_cycle_backtest_filtered_v2.py --csv data.csv --symbols BTC-USD ETH-USD XRP-USD SOL-USD \
  --lookback_days 45 --risk basis=range tp_mult=1.0 sl_mult=0.75 \
  --whitelist whitelist.csv --edge_threshold 0.002 --out_prefix session_cycle_filtered_edge
"""
import argparse
import os
...
def parse_args():
    p = argparse.ArgumentParser(description="Time-of-Day & Liquidity Cycle Backtester (Filtered + Edge Threshold)")
    ...
    p.add_argument("--out_prefix", type=str, default="session_cycle_filtered_edge")

    p.add_argument("--whitelist", type=str, default=None, help="CSV with columns symbol,session to allow")
    p.add_argument("--edge_threshold", type=float, default=0.0, help="Absolute mean return threshold per lookback (e.g., 0.002 = 0.2%)")
    ...
    for sym in symbols:
        ...
        for sname in SESSION_DEFS.keys():
            ...
            for i, day in enumerate(days):
                ...
                if abs(mean_ret) < edge_threshold:
                    continue  # skip weak edge windows
...
def main():
    ...
    trades, session_stats = backtest(
        df, symbols, args.lookback_days, args.min_days_for_trade,
        risk_cfg, args.one_trade_per_coin_per_day, allow, args.edge_threshold
    )
