...
def merge_btc(df, granularity=GRANULARITY):
    """
    Use the prebuilt BTC CSV from the training pipeline so we get the exact
    btc_* columns (including btc_label/tp_pct/sl_pct) that the model expects.
    Falls back to live-compute if the file is missing.
    """
    btc_csv = os.path.join(DATA_DIR, "BTC_USD.csv")  # created by dataset_builder.py
    end = df["time"].max()

    if os.path.exists(btc_csv):
        # BTC file already has 'time' + btc_* columns (trainer saved with btc_ prefix)
        btc = pd.read_csv(btc_csv)
        btc["time"] = pd.to_datetime(btc["time"], utc=True)
        btc = btc.sort_values("time")
        # align up to the latest time we have for the coin
        btc = btc[btc["time"] <= end]
        # asof-merge retains exact training names like btc_label/btc_tp_pct/btc_sl_pct
    ...
    # ---- Fallback (if CSV missing): rebuild live BTC features ----
    # NOTE: this path won't include btc_label/tp/sl since they're lookahead-based.
    # Only use if you must; better to generate BTC_USD.csv with dataset_builder.py.
    ...
    # use prefixed fibs to match trainer naming
...
def calc_fib_prefixed(df, lookback, prefix):
    """
    Match dataset_builder's naming:
    fib_long_*, fib_med_*, fib_short_ (NOT fib_720_*).
    """
    swing_high = df["high"].rolling(lookback, min_periods=1).max()
    swing_low  = df["low"].rolling(lookback, min_periods=1).min()
    diff = swing_high - swing_low
...
class CoinContext:
    ...
    def _latest_features(self):
        ...
        start = now - timedelta(hours=self.seq_l + 800)  # plenty of warmup
    ...
    # Build features
        ...
        # --- Ensure base OHLCV + time features exist and are numeric ---
        required_base = ["open","high","low","close","volume"]
        for col in required_base:
            if col not in df.columns:
                raise RuntimeError(f"{self.coin_tag}: live OHLCV column missing: {col}")
    # coerce to numeric just in case
            df[col] = pd.to_numeric(df[col], errors="coerce")

# (Re)build normalized time features to match training
        if "time" not in df.columns:
            raise RuntimeError(f"{self.coin_tag}: 'time' missing pre-sanitization")
    ...
    # Sanitize: handle inf/NaN from indicators, then enforce exact training cols
    ...
    # Enforce exact training feature set & order
        ...
        missing = [c for c in self.feature_cols if c not in df_num.columns]
        if missing:
            raise RuntimeError(f"{self.coin_tag}: live frame missing training cols, e.g. {missing[:6]}")

        df_num = df_num.reindex(columns=self.feature_cols)

    # Drop any remaining NaN rows (indicator warmups) BEFORE scaling/windowing
    ...
    # Final guard against NaNs/Infs entering the model
        ...
        X, y_idx = make_windows_multiscale(
            X_scaled, self.feature_cols, self.seq_s, self.seq_l, self.ds
        )
    # Return the very last window, and align to original df for bar metadata
        last_idx = y_idx[-1]
    ...
    def infer(self):
        ...
        # Softmax -> probabilities for [HOLD, LONG, SHORT]
            ...
            entry_price = float(bar.get("close", np.nan))  # close of the bar used
...
def main():
    ...
    # Force all models to use threshold 0.80
    for ctx in contexts:
        ctx.threshold = prob_t
        print(f"[THR] {ctx.coin_tag}: forced threshold {ctx.threshold:.2f}")


    print("[STARTUP] Running immediate scan before switching to hour-close mode...")
    for ctx in contexts:
        try:
            sig = ctx.infer()
            ...
            if sig["status"] == "ALERT":
                ...
                print(msg)
                # send_telegram(msg)  # uncomment if you want immediate-run to notify
            else:
    ...
    print("[WATCH] Hour-close mode enabled — running at the top of each hour.")
...
# Only act when we see a NEW bar vs the last one we processed
    ...
    # Pull probs for display (present for both HOLD and ALERT)
                            pH = sig.get("prob_hold", 0.0)
                            pL = sig.get("prob_long", 0.0)
                            pS = sig.get("prob_short", 0.0)
                            print(
        ...
        # Telegram (kept clean; no probs unless you want them)
                                tg_text = (
                                f"{ctx.coin_sym} {side}\n"
                                f"Time: {bar_time}\n"
