...
def train_coin(coin_file):
    ...
    for epoch in range(EPOCHS):
        ...
        # Coarse search with trade floor
        best_epoch_profit_now = -1e9
        best_epoch_trades_now = 0
        for thr in THRESH_COARSE:
            p, wr, n = grid_eval(maxp, argmax, closes_val, highs_val, lows_val, tp_preds, sl_preds, thr)
            if n < TRADE_MIN:  # enforce min trades
                continue
            if p > best_epoch_profit_now:
                best_epoch_profit_now = p
                best_epoch_trades_now = n

        # Fallback if all thresholds fail the floor: pick best profit, tie-break by higher trades
        if best_epoch_profit_now == -1e9:
            fallback_p, fallback_n = -1e9, 0
            for thr in THRESH_COARSE:
                p, wr, n = grid_eval(maxp, argmax, closes_val, highs_val, lows_val, tp_preds, sl_preds, thr)
                if (p > fallback_p + 1e-12) or (abs(p - fallback_p) <= 1e-12 and n > fallback_n):
                    fallback_p, fallback_n = p, n
            best_epoch_profit_now, best_epoch_trades_now = fallback_p, fallback_n

        print(f"{coin} | Epoch {epoch+1}/{EPOCHS} | Val Profit(coarse): {best_epoch_profit_now*100:.4f}% | Trades*: {best_epoch_trades_now}")

        if best_epoch_profit_now > best_val_profit + 1e-12:
            best_val_profit = best_epoch_profit_now
            best_epoch = epoch + 1
