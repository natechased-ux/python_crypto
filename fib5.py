...
# Paths
LOG_PATH   = os.getenv("TRADE_LOG_PATH", "live_trade_log.csv")
THR_PATH   = os.getenv("LIQ_THRESHOLDS_PATH", "thresholds.json")
...
WARMUP_HOURS                 = int(os.getenv("WARMUP_HOURS", "1"))
...
STRICT_LONGS        = os.getenv("STRICT_LONGS", "true").lower() == "true"
LONG_REQUIRE_TREND  = os.getenv("LONG_REQUIRE_TREND", "true").lower() == "true"
LONG_MIN_CONF       = int(os.getenv("LONG_MIN_CONF", "3"))       # require ≥3 active zones if no Fib
LONG_DELTA_MULT     = float(os.getenv("LONG_DELTA_MULT", "1.75"))# strength of delta vs per-coin threshold
FIB_TINY_MIN_CONF_FOR_ALERT = int(os.getenv("FIB_TINY_MIN_CONF_FOR_ALERT", "3"))
# requires ≥ this many active zones when the active zone is FIB_TINY
import numpy as np  # <<< add

STOCH_FILTER_CONT_SHORTS = os.getenv("STOCH_FILTER_CONT_SHORTS", "true").lower() == "true"
...
def _stoch_rsi_last(close: pd.Series, rsi_len=14, stoch_len=14, k_len=3, d_len=3):
    ...
    if k.dropna().shape[0] < 2 or d.dropna().shape[0] < 2:
...
import re
MIN_STARS_FOR_ALERT = int(os.getenv("MIN_STARS_FOR_ALERT", "2"))  # only send alerts with ≥ this many stars

def _stars_from_msg(msg: str) -> int:
    m = re.search(r"Strength:\s*\*([★☆]{3})\*", msg)
    return m and m.group(1).count("★") or 0
...
def check_signal(product_id: str, cooldowns: dict, last_alert_key: dict):
    ...
    # >>> Option A (ONLY ALERT 2★+): env-configurable star gate for sending
    MIN_STARS_FOR_ALERT = int(os.getenv("MIN_STARS_FOR_ALERT", "2"))

    # Candles
    df1h = fetch_candles(product_id, 3600)
    df6h = fetch_candles(product_id, 21600)
    df1d = fetch_candles(product_id, 86400)
    df15m = fetch_candles(product_id, 900)
    if len(df1h) < 7*24 + 2: return None
    ...
    if last and (now - last).total_seconds() < COOLDOWN_MINUTES*60: return None
    ...
    # Adaptive mode & side
    mode = choose_mode(liq_reason, float(liq_diag.get("delta_usd",0.0)), edge_now)
    if ENABLE_CONTINUATION and mode == "CONTINUATION":
        if (liq_diag.get("delta_usd",0.0) >= 0 and edge_now in {"upper","middle"}): side = "LONG"
        elif (liq_diag.get("delta_usd",0.0) < 0 and edge_now in {"lower","middle"}): side = "SHORT"
        # regime bias
        if regime == "range":
            if abs(liq_diag.get("delta_usd",0.0)) < LIQ_THRESHOLDS[product_id]["delta_usd"]*1.25:
                return None
        if WEEKEND_DISABLE_CONT and is_weekend_utc(now):
            return None
    else:
        mode = "REVERSAL"
    ...
    if b_near==0 and a_near==0: return None

    # --- Stoch RSI (annotation + later send-gate for CONT SHORT) ---
    k1h = d1h = k1h_prev = d1h_prev = None
    k15 = d15 = k15_prev = d15_prev = None
    if not df1h.empty:
        k1h, d1h, k1h_prev, d1h_prev = _stoch_rsi_last(df1h["close"])
    if not df15m.empty:
        k15, d15, k15_prev, d15_prev = _stoch_rsi_last(df15m["close"])

    oneH_k_gt_d   = (k1h is not None and d1h is not None and k1h > d1h)
    cross15m_down = (k15_prev is not None and d15_prev is not None and k15 is not None and d15 is not None
                 and (k15_prev >= d15_prev) and (k15 < d15))
    stoch_bonus   = oneH_k_gt_d and cross15m_down
    ...
    if USE_SL_CAP:
        if side == "LONG":
            ...
            sl_price = max(sl_price, sl_cap)  # never wider than cap
        else:
            sl_cap = entry * (1 + SL_PCT_MAX)
            sl_price = min(sl_price, sl_cap)

    # --- Stars per your mapping (Fib / Confluence) ---
    ...
    if tp2_price: msg += f" | TP2: ${fmt_price(tp2_price)}"
    msg += f"\n{msg_plan}\nLiquidity: {liq_line}\nTime: {now.strftime('%Y-%m-%d %H:%M UTC')}"

    stoch_line = []
    if k1h is not None and d1h is not None:
        stoch_line.append(f"1H K/D={k1h:.1f}/{d1h:.1f}{' (K>D)' if oneH_k_gt_d else ''}")
    if k15 is not None and d15 is not None:
        stoch_line.append(f"15m K/D={k15:.1f}/{d15:.1f}{' (cross↓)' if cross15m_down else ''}")
    if stoch_line:
        msg += "\nStochRSI: " + " • ".join(stoch_line)
    if stoch_bonus:
        msg += " • Stoch bonus (1H K>D + 15m cross↓)"


    # Log + cooldown + dedupe
    stop_type = "LIQ" if "liquidity" in msg_plan.lower() else stop_type
    # Persist Stoch RSI into CSV via liq_diag (Fib5)
    liq_diag["stoch1h_k"] = k1h
    liq_diag["stoch1h_d"] = d1h
    liq_diag["stoch1h_k_gt_d"] = int(1 if (k1h is not None and d1h is not None and k1h > d1h) else 0)

    liq_diag["stoch15m_k"] = k15
    liq_diag["stoch15m_d"] = d15
    liq_diag["stoch15m_cross_up"]  = int(1 if (k15_prev is not None and d15_prev is not None and k15 is not None and d15 is not None and k15_prev <= d15_prev and k15 > d15) else 0)
    liq_diag["stoch15m_cross_dn"]  = int(1 if (k15_prev is not None and d15_prev is not None and k15 is not None and d15 is not None and k15_prev >= d15_prev and k15 < d15) else 0)
    ...
    # --- Stoch filter for CONTINUATION SHORT alerts (log always, gate send) ---
    if STOCH_FILTER_CONT_SHORTS and mode == "CONTINUATION" and side == "SHORT":
        if not (oneH_k_gt_d or cross15m_down):
            print(f"[ALERT muted] {product_id} CONT SHORT stoch filter (need 1H K>D OR 15m cross↓)")
            return None


    # >>> Option A star-gate at return (keeps logging + cooldowns, mutes Telegram)
        # --- FIB_TINY confluence gate (mute alert, keep logging) ---
    if zone_source == "FIB_TINY" and zone_count < FIB_TINY_MIN_CONF_FOR_ALERT:
        print(f"[ALERT muted] {product_id} {side} FIB_TINY needs ≥{FIB_TINY_MIN_CONF_FOR_ALERT} zones (had {zone_count})")
        return None

    # >>> Option A star-gate at return (keeps logging + cooldowns, mutes Telegram)
    star_count = stars.count("★")
    if star_count < MIN_STARS_FOR_ALERT:
        print(f"[ALERT muted <{MIN_STARS_FOR_ALERT}★] {product_id} {side} {star_count}★ | zone={zone_source}")
        return None
...
def main_loop():
    ...
    # Optional coin rules
    try:
        from coin_rules import set_all_coins, load_rules
        set_all_coins(COINS); load_rules()
    except Exception:
        pass
    ...
    lock = threading.Lock()
    tuner = AutoTuner(LIQ_THRESHOLDS, lock); tuner.start()
    outmon = OutcomeMonitor(); outmon.start()

    print("Fib5 (Kraken) — streaming, adaptive REV/CONT, liquidity exits, tuner, outcomes, diagnostics")
    print_zone_diagnostics(COINS)
    debug_top_candidates(COINS)

    while True:
        start = time.time()
        for coin in COINS:
            # In main_loop(), inside the for coin in COINS loop:
            try:
                alert = check_signal(coin, cooldowns, STATE.get("last_alert_key", {}))
                if alert:
                    if _stars_from_msg(alert) >= MIN_STARS_FOR_ALERT:
                        send_telegram(alert)
                    else:
                        print(f"[ALERT muted <{MIN_STARS_FOR_ALERT}★] {coin}")
        ...
        # Console diagnostics
        if (time.time() - last_diag) >= ZONE_DIAG_INTERVAL_SEC:
            print_zone_diagnostics(COINS)
            debug_top_candidates(COINS)
            last_diag = time.time()

        # Health alarm (no alerts + no coins in zones for a long time)
        if (time.time() - last_health) > HEALTH_ALERT_NOALERTS_H*3600:
            any_hits = False
            for c in COINS:
                try:
                    if zones_touching_now(c): any_hits=True; break
                except: pass
            if not any_hits:
                print("[HEALTH] No alerts recently & no coins in zones — thresholds may be too strict or data stale.")
                send_telegram("_Health_: No alerts & no coins in zones; consider loosening thresholds or checking data feed.")
            last_health = time.time()

        # pacing
        dt = time.time() - start
        if dt < INTERVAL_SECONDS: time.sleep(INTERVAL_SECONDS - dt)
