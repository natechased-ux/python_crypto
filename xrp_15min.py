...
minute_data = []  # Stores raw data for the current 15-minute period
price_data = deque(maxlen=RSI_PERIOD + SMA_PERIOD)  # Stores 15-minute close prices
...
# === AGGREGATION ===
def aggregate_15min_data():
    global minute_data
    if not minute_data:
        return None

    # Aggregate to create a 15-minute "candle"
    ...
    minute_data = []  # Reset for the next 15-minute period
    return candle
...
def on_message(ws, message):
    ...
    if msg['event'] == 'data':
        ...
        # Add raw data to 15-minute aggregation
        minute_data.append((timestamp, last_price))

        # Check if a new 15-minute period has started
        if len(minute_data) > 1 and (minute_data[-1][0].minute // 15 != minute_data[-2][0].minute // 15):
            candle = aggregate_15min_data()
            if candle:
        ...
        # Calculate indicators based on 15-minute close prices
