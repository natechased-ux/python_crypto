...
# -----------------------------
def simulate_profit(pred_labels, closes, highs, lows, tp_preds, sl_preds, lookahead=8):
    """
...
# Compute dynamic class weights
    ...
    for epoch in range(num_epochs):
        ...
        confidence_threshold = 0.30  # minimum probability to consider a trade
...
# Now run simulate_profit only on confident trades
        val_profit, win_rate, active_trades = simulate_profit(
            ...
            tp_preds_f, sl_preds_f, lookahead=8
        )
...
# Distribution stats
    ...
    # Save only model weights
            save_path = os.path.join(model_save_dir, f"{coin_symbol}_lstm_tp_sl.pth")
...
if __name__ == "__main__":
    train_all_datasets("datasets_macro_training4", "models_lstm4",
                       seq_len=60, batch_size=64, lr=1e-3, num_epochs=50)
