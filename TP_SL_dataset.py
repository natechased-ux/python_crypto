...
TP_MULTIPLIER = 0.75
LOOKAHEAD_CANDLES = 2         # 4 × 6h = 24h forward look for labeling
...
def label_trades(df, lookahead=LOOKAHEAD_CANDLES):
    ...
    for i in range(len(df) - lookahead):
        ...
        # Assign label based on profitable move
        if mfe_long > mfe_short:  # Long better
            if mfe_long > 0: label = 1
            else: label = 0
            mfe_list.append(mfe_long)
            mae_list.append(abs(mae_long))
        else:  # Short better
            if mfe_short > 0: label = -1
            else: label = 0
...
if all_data:
    ...
    dataset.to_csv("lstm_training_dataset_tp_sl.csv", index=False)
    print(f"✅ Dataset saved: {dataset.shape[0]} rows, {dataset.shape[1]} columns → lstm_training_dataset_tp_sl.csv")
else:
