...
price_range_percentage = .25  # Must be this close to trigger an alert
volume_spike_multiplier = 1
risk_reward_ratio = 1.5
whale_cluster_percentile = 95
max_price_distance_percent = 10  # Max considered range for clustering
...
def analyze_whale_clusters(symbol, price):
    ...
    print(f"[{symbol}] Filtered bids: {len(bids)}, asks: {len(asks)}")

    grouped_bids = group_orders(bids, aggregation_threshold)
    grouped_asks = group_orders(asks, aggregation_threshold)

    print(f"[{symbol}] Grouped bids: {len(grouped_bids)}, asks: {len(grouped_asks)}")
    ...
    print(f"[{symbol}] Whale bids: {len(whale_bids)}, whale asks: {len(whale_asks)}")

    # Final alert-range filter
    filtered_bids = [b for b in whale_bids if 0 < (price - b[0]) / price * 100 <= price_range_percentage]
    filtered_asks = [a for a in whale_asks if 0 < (a[0] - price) / price * 100 <= price_range_percentage]

    print(f"[{symbol}] ALERT-RANGE bids: {len(filtered_bids)}, asks: {len(filtered_asks)}")

    bid = min(filtered_bids, key=lambda x: abs(x[0] - price), default=None)
    ask = min(filtered_asks, key=lambda x: abs(x[0] - price), default=None)
...
# Whale cluster visualization (shows all whale bids/asks within 10%)
...
# Uncomment to test visualization manually
