...
ALL_COINS = [
    ...
    "DRIFT-USD", "KAVA-USD", "SPA-USD", "BAT-USD",
...
# =====================================================
USER_PROMPT = """
You are an elite crypto trend continuation trader specializing in bull flags, ascending wedges, and consolidation breakouts.

You are given coins that already meet ALL of the following baseline conditions:
- 6H EMA stack is bullish (EMA25 > EMA50)
- EMAs are rising or flat-to-rising (not rolling over)
- Price is above EMA25
- Price is not parabolic or heavily extended

Your task:
1. Identify coins that are forming HIGH-QUALITY bull flags, rising wedges (continuation wedges), or tight consolidation patterns after a prior impulse.
2. Focus on early-to-mid trend continuation setups — NOT late-stage, blow-off, or exhaustion moves.
3. Rank ALL coins from MOST technically bullish to LEAST bullish.
4. Select ONLY the TOP 7 best long continuation setups.

A valid bull flag / wedge setup typically includes:
- A clear prior impulsive move establishing bullish trend context
- A controlled consolidation or pullback with:
  - Higher lows, parallel flag structure, or contracting wedge boundaries
  - No decisive breakdown below key EMAs
- Volatility contraction during consolidation (ATR compression or narrowing price range)
- Volume declining or stabilizing during the flag/wedge, NOT expanding on the pullback
- RSI resetting or holding in a bullish neutral zone (45–65), not overbought
- Price respecting EMA25 or EMA50 as dynamic support during the pattern

Avoid:
- Late-stage trends with multiple expansion legs and no meaningful consolidation
- Flags or wedges that are already breaking out or extended far from EMAs
- Rising wedges that show distribution characteristics (heavy sell volume or failed EMA holds)

For each selected setup, provide:
- symbol
- setup_quality (A+, A, or B)
- pattern_type (bull flag, ascending wedge, flat consolidation)
- ideal_entry (flag low, wedge support, EMA pullback, or breakout trigger, with price)
- stop_loss (below flag low, wedge invalidation, or EMA structure, with price)
- take_profits (measured move from the flag/wedge or trend continuation targets, with price)
- rationale (prior impulse strength, pattern structure, EMA behavior, volatility contraction, RSI state, and volume behavior)

Important rules:
- Penalize coins that appear to be in a second or late impulse unless clear re-accumulation is present.
- Favor tight, orderly consolidations over loose or volatile pullbacks.
- Prefer patterns that have not yet broken out or are just beginning to resolve.
- Avoid overbought RSI (>70) unless volume contraction and structure strongly favor continuation.
