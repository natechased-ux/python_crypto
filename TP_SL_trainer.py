...
EPOCHS = 50
LR = 0.001
MODEL_PATH = "lstm_tp_sl_model.pt"

# === LOAD DATA ===
df = pd.read_csv("lstm_training_dataset_3candle.csv")
...
class LSTM_TP_SL(nn.Module):
    def __init__(self, input_size, hidden_size=64, num_layers=2, output_size=2):
        ...
        self.fc = nn.Linear(hidden_size, output_size)  # TP & SL
    def forward(self, x):
        h, _ = self.lstm(x)
        return self.fc(h[:, -1, :])

model = LSTM_TP_SL(input_size=len(FEATURES))
...
print(f"✅ TP/SL Model saved to {MODEL_PATH}")
