...
# Per-coin TP/SL % config (guardrailed winners)
WINNER_TPSL_PATH = os.getenv("WINNER_TPSL_PATH", "winner_tpsl_percent.json")
try:
    with open(WINNER_TPSL_PATH, "r") as _tf:
        _TPSL = json.load(_tf)
except Exception:
    _TPSL = {"tp_sl_suggestions": {}}

TPSL_PCT = _TPSL.get("tp_sl_suggestions", {})


# Winners/Policy config
WINNER_CFG_PATH = os.getenv("WINNER_CFG_PATH", "winner_alerts_config.json")
try:
    with open(WINNER_CFG_PATH, "r") as _wf:
        _WINNER_CFG = json.load(_wf)
except Exception:
    _WINNER_CFG = {"winners_weekday": [], "policy": {"alerts_weekdays_only": True, "alerts_q34_flow_only": True, "log_every_candidate": True}}

WINNERS_WEEKDAY = set(_WINNER_CFG.get("winners_weekday", []))
POLICY = _WINNER_CFG.get("policy", {})
POLICY_WEEKDAYS_ONLY = bool(POLICY.get("alerts_weekdays_only", True))
POLICY_Q34_ONLY = bool(POLICY.get("alerts_q34_flow_only", True))
POLICY_LOG_ALL = bool(POLICY.get("log_every_candidate", True))
...
def rsi_wilder(series: pd.Series, length: int = 14) -> pd.Series:
    ...
    # ensure numeric dtype first (avoids object→numeric downcast surprises)
    rsi = pd.to_numeric(rsi, errors="coerce")

# backfill gaps, then fill any remaining with neutral 50.0
    rsi = rsi.bfill().fillna(50.0)

# make dtype explicit (future-proof against silent downcasting changes)
    return rsi.astype("float64", copy=False)
...
def check_signal(product_id: str, cooldowns: dict, last_alert_key: dict):
    ...
    # --- Safe access to policy + winners + per-coin TP/SL% (fallbacks if globals not loaded) ---
    WINNERS_WEEKDAY      = globals().get("WINNERS_WEEKDAY", set())
    POLICY_WEEKDAYS_ONLY = bool(globals().get("POLICY_WEEKDAYS_ONLY", True))
    POLICY_Q34_ONLY      = bool(globals().get("POLICY_Q34_ONLY", True))
    TPSL_PCT             = globals().get("TPSL_PCT", {})  # {"SYMBOL":{"TP_pct":..,"SL_pct":..,"trail_on_R":..}}
    ...
    # === Combined alert policy: C1–C3 OR Swing (12h bias) ===
    _sells = float(liq_diag.get("sells_usd", 0.0))
    _buys  = float(liq_diag.get("buys_usd", 0.0))
    _delta = float(liq_diag.get("delta_usd", 0.0))
    _symU  = str(product_id).upper()
    _zoneU = str(zone_source).upper()
    _sideU = str(side).upper()

    # C1–C3 combos
    _C1 = (_sells <= 143.0) and (_zoneU == "EMA50_1D")
    _C2 = (_sideU == "LONG") and (_zoneU == "EMA50_1D")
    _C3 = (_sells <= 143.0) and (_symU in {"UNI/USD","HBAR/USD"})
    _is_c1c3 = bool(_C1 or _C2 or _C3)

    # Swing combos (use already-fetched 6H/1D/1H data for stoch RSI)
    try:
        k6h, d6h = stoch_rsi_kd(df6h["close"])
    except Exception:
        k6h, d6h = float("nan"), float("nan")
    try:
        k1d, d1d = stoch_rsi_kd(df1d["close"])
    except Exception:
        k1d, d1d = float("nan"), float("nan")
    try:
        k1h, d1h = stoch_rsi_kd(df1h["close"])
    except Exception:
        k1h, d1h = float("nan"), float("nan")

    SW1 = (abs(_delta) >= 4886.576) and (d6h == d6h) and (6.0 <= d6h <= 46.0)
    SW2 = (_buys >= 5690.21) and (k1d == k1d) and (k1d <= 30.0)
    SW3 = (_sells <= 143.452) and (d1h == d1h) and (17.5 <= d1h <= 34.3)
    _is_swing = bool(SW1 or SW2 or SW3)

    # decide if we proceed to alert; if neither, stop here
    _proceed_alert = (_is_swing or _is_c1c3)
    if not _proceed_alert:
        return None
    ...
    # --- Flow features & quintiles (annotation) ---
    ...
    if ("quintile_edges" not in STATE) or (now.timestamp() - STATE.get("q_last_ts", 0)) > 300:
        ...
        # Ensure JSON-safe lists in STATE
        def _tolist(x):
            import numpy as np
    ...
    # --- Swing12hSimple TP/SL override (only for swing alerts) ---
    try:
        if _is_swing:
            _risk = abs(entry - sl_price)
            if _risk and _risk > 0:
                _TP_R = 1.75; _SL_R = 1.75
                if side == "LONG":
                    sl_price = entry - (_SL_R * _risk)
                    tp1_price = entry + (_TP_R * _risk)
                else:
                    sl_price = entry + (_SL_R * _risk)
                    tp1_price = entry - (_TP_R * _risk)
                stop_type = "Swing12hSimple"
                msg_plan = f"{msg_plan} | Swing12hSimple TP={_TP_R:.2f}R SL={_SL_R:.2f}R"
    except Exception as _e:
        print("[Swing12hSimple] override failed:", _e)

    # ---------------------------------------------
    # Winner policy gating (ALERT policy only here)
    #  - we still LOG regardless of gating result
    #  - we apply per-coin % TP/SL to the alert message
    # ---------------------------------------------
    should_alert = True
    if POLICY_WEEKDAYS_ONLY and is_weekend_utc(now):
        should_alert = False
    if WINNERS_WEEKDAY and (product_id not in WINNERS_WEEKDAY):
        should_alert = False
    if POLICY_Q34_ONLY and (abs_delta_q_coin not in (4,5)):
        should_alert = False

    # ---- Apply per-coin % TP/SL for the alert message (highest-R config) ----
    used_pct_tpsl = False
    coin_cfg = TPSL_PCT.get(product_id)
    if coin_cfg and isinstance(coin_cfg.get("TP_pct"), (int,float)) and isinstance(coin_cfg.get("SL_pct"), (int,float)):
        TP_pct = coin_cfg["TP_pct"]; SL_pct = coin_cfg["SL_pct"]
        if TP_pct > 0 and SL_pct > 0:
            used_pct_tpsl = True
            if side.upper() == "LONG":
                sl_price = curr * (1 - SL_pct/100.0)
                tp1_price = curr * (1 + TP_pct/100.0)
            else:
                sl_price = curr * (1 + SL_pct/100.0)
                tp1_price = curr * (1 - TP_pct/100.0)
    ...
    # Alert message
    # -------------
    alert_type = "SWING" if ("_is_swing" in locals() and _is_swing) else "C1-C3"
    msg = f"{alert_type} | {product_id} | Entry: ${fmt_price(curr)} | TP: ${fmt_price(tp1_price)} | SL: ${fmt_price(sl_price)}"
    ...
    liq_diag["used_pct_tpsl"] = int(used_pct_tpsl)

    stop_type = "LIQ" if "liquidity" in msg_plan.lower() else stop_type
    log_trade(product_id, side, mode, curr, sl_price, tp1_price, tp2_price, stop_type, zone_source, zones_list, stars, liq_reason, liq_diag, now)

    # -------------
    # Cooldown & send (only if policy allows)
    # -------------
    if not should_alert:
        return None
