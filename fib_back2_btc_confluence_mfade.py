...
def momentum_fade_ok(side: str, rsi_series: pd.Series, hist_series: pd.Series, idx: int, window: int) -> tuple[bool, float, float]:
    ...
    if side == "LONG":
        ...
        ok = ((not np.isnan(rsi_slope) and rsi_slope > 0) if {MFADE_USE_RSI} else True) and \             ((not np.isnan(hist_slope) and hist_slope > 0) if {MFADE_USE_MACD} else True)
    else:
