...
def get_dynamic_whale_threshold(symbol):
    ...
    mid_value_coins = ["ltc-usd", "ada-usd", "sol-usd", "link-usd", "dot-usd", "qnt-usd", "avax-usd", "near-usd", "uni-usd", "algo-usd"]

    low_value_coins = ["xrp-usd", "doge-usd", "sei-usd", "magic-usd", "ape-usd", "jasmy-usd", "wld-usd", "hbar-usd", "crv-usd", "inj-usd", 
...
def analyze_order_book(symbol, data):
    try:
        ...
        if nearest_bid and nearest_ask:
            if abs(nearest_ask[0] - nearest_bid[0]) / close_price < 0.02:  # Example: 2% zone
                return None
        ...
        if long_signal:
            ...
            sl = close_price - atr
            return (f"#5{symbol.upper()}: LONG Signal | Entry: {format_price(close_price)}, TP: {format_price(tp)}, SL: {format_price(sl)}\n"
                    f"Support (Nearest Bid): {format_price(nearest_bid[0])}")
        elif short_signal:
            tp = close_price - (atr * risk_reward_ratio)
            sl = close_price + atr
            return (f"#5{symbol.upper()}: SHORT Signal | Entry: {format_price(close_price)}, TP: {format_price(tp)}, SL: {format_price(sl)}\n"
