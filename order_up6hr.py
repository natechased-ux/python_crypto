...
def analyze_order_book(symbol, data):
    try:
        ...
        if long_signal:
            ...
            sl2 = close_price - (atr)
            return (f"6hr{symbol.upper()}: LONG Signal | Entry: {format_price(close_price)}, TP1: {format_price(tp1)}, TP2: {format_price(tp2)}\n"
            ...
            sl1 = close_price + (atr*.5)
            sl2 = close_price + atr
            return (f"6hr{symbol.upper()}: SHORT Signal | Entry: {format_price(close_price)}, TP1: {format_price(tp1)}, TP2: {format_price(tp2)}\n"
                    f"SL1: {format_price(sl1)}, SL2: {format_price(sl2)}\n"
...
async def hourly_updates():
    while True:
        ...
        print(f"⏰ Still running: checked {len(symbols)} symbols — {pd.Timestamp.now()}")
