...
SAFETY_DELAY_MIN = 2  # Wait 3 min after close


import time
import telegram.error

def safe_send_message(bot, chat_id, text, parse_mode="Markdown"):
    for attempt in range(3):
        try:
            # Explicitly call the Telegram API method, not our own wrapper
            telegram.Bot.send_message(bot, chat_id=chat_id, text=text, parse_mode=parse_mode)
            return
...
def log_trade(symbol, side, conf, entry, tp, sl, candle_time):
    log_exists = os.path.exists(TRADE_LOG_FILE)
    with open(TRADE_LOG_FILE, "a", newline="") as f:
        writer = csv.writer(f)
        if not log_exists:
            writer.writerow([
                "trade_id", "timestamp_utc", "symbol", "direction",
                "entry", "tp", "sl", "confidence", "candle_time",
                "high_since_entry", "low_since_entry", "status"
            ])
        trade_id = f"{symbol}_{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
        writer.writerow([
            trade_id,
            ...
            round(conf, 4),
            candle_time,
            "",  # high_since_entry (will fill in later)
            "",  # low_since_entry (will fill in later)
            "OPEN"
        ])
    print(f"📝 Logged trade for {symbol} ({side}) → ID: {trade_id}")
...
def check_ai_signals():
    ...
    for symbol in COINS:
        ...
        # ✅ Drop incomplete candle before model input

# === LOCK TO LAST OFFICIAL 6H CLOSE ===
# Make sure times are UTC aware
        if df["time"].dt.tz is None:
            df["time"] = df["time"].dt.tz_localize("UTC")

        now_utc = datetime.utcnow().replace(tzinfo=pytz.UTC)

# Find last official 6h close
        hour_block = (now_utc.hour // 6) * 6
        last_official_close = now_utc.replace(hour=hour_block, minute=0, second=0, microsecond=0)

# If we are before that close time (because we're mid‑block), step back one
        if now_utc < last_official_close:
            last_official_close -= timedelta(hours=6)

# Filter strictly to candles <= last official close
        df_closed = df[df["time"] <= last_official_close].copy()

# Make sure we actually have the candle
        if df_closed.empty:
            print(f"{symbol} → No data for last official close {last_official_close}")
            continue

# Sort and select last official closed candle
        df_closed.sort_values("time", inplace=True)
        last_closed_time = df_closed["time"].iloc[-1]
        last_price = df_closed["close"].iloc[-1]
        candle_time_str = last_closed_time.strftime("%Y-%m-%d %H:%M UTC")

        print(f"✅ {symbol} → Using official 6h close {candle_time_str} | Close: {last_price:.6f}")
        ...
        # In check_ai_signals(), after calculating long_prob / short_prob
        if max(long_prob, short_prob) >= 0.70:
    # Determine direction for logging
            ...
            log_trade(symbol, log_dir, log_conf, last_price, tp_price, sl_price, candle_time_str)
    ...
    safe_send_message(bot,chat_id=CHAT_ID, text=bias_msg, parse_mode="Markdown")
...
def filter_trades_by_bias(all_signals, top_n=10, high_conf_cutoff=0.90, min_bias_ratio=0.60, bias_conf_min=0.70):
    ...
    if total == 0:
        ...
        safe_send_message(bot,chat_id=CHAT_ID, text=msg, parse_mode="Markdown")
