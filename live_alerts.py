...
# -*- coding: utf-8 -*-
"""
Live Alerts for High-Conviction Strategy (ADX 25, per-symbol TP/SL)

- Trend/structure on 6H:
    * EMA200 filter + MACD histogram sign
    * ADX >= adx_min (default 25)
    * RSI filters (long >= 55, short <= 45)
- Confirmation on 1H within confirm_hours (default 12h):
    Medium mode  :  StochRSI (relaxed) OR (breakout OR pullback)
    Strict-lite  :  StochRSI (relaxed) AND (breakout OR pullback)
- Optional FVG confluence on 6H
- TP/SL from tp_sl_config.json (TP=MFE p60, SL=MAE p70), with ATR guardrails & risk cap
- Polls Coinbase public API (no keys) on a schedule

Run examples:
  py live_alerts.py --symbols BTC-USD ETH-USD SOL-USD --interval 5 --mode strictlite
  py live_alerts.py --use_fvg --out alerts.csv

Tip: keep tp_sl_config.json in the same folder.
"""
...
def fetch_candles(product_id: str, start: pd.Timestamp, end: pd.Timestamp, granularity: int) -> pd.DataFrame:
    """Fetch candles from Coinbase in chunks (public REST)."""
    step = pd.Timedelta(seconds=granularity * 300)
    frames = []
    headers = {"User-Agent": "live-alerts/1.0"}
    t0 = start
    while t0 < end:
        t1 = min(t0 + step, end)
...
# -------------------- Alert Engine --------------------
def format_alert(symbol: str, side: str, entry: float, tp: float, sl: float, meta: Dict) -> str:
    rr = (tp - entry) / (entry - sl) if side == "long" else (entry - tp) / (sl - entry)
    return (f"[{pd.Timestamp.utcnow().isoformat()}] {symbol} {side.upper()} "
            f"entry={entry:.6f} tp={tp:.6f} sl={sl:.6f} RR~{rr:.2f} | "
            f"mode={meta['mode']} adx_min={meta['adx_min']} use_fvg={meta['use_fvg']}")
...
def main():
    ...
    if not os.path.exists(args.out):
        pd.DataFrame(columns=[
            "ts","symbol","side","entry","tp","sl","mode","adx_min","use_fvg","trigger_time","htf_time"
    ...
    while True:
        ...
        for symbol in args.symbols:
            try:
                ...
                meta = {"mode": args.mode, "adx_min": args.adx_min, "use_fvg": args.use_fvg}
                line = format_alert(symbol, side, entry, tp, sl, meta)
                print(line)
        ...
        sleep_s = max(5, args.interval*60 - elapsed)
        time.sleep(sleep_s)
