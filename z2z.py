...
# ===== Networking: global session + retries + rate limit =====
SESS = None
_SESS_LOCK = threading.Lock()
_LAST_REQ_TS = 0.0
RATE_LIMIT_QPS = 2.0   # max ~2 requests per second across threads
...
def get_session():
    ...
    with _SESS_LOCK:
        if SESS is None:
            ...
            s.headers.update({"User-Agent": "zone2zone-winners-bot/1.0"})
            SESS = s
        return SESS
...
def throttle():
    ...
    with _SESS_LOCK:
        ...
        if delta < min_gap:
            _t.sleep(min_gap - delta)
        _LAST_REQ_TS = _t.time()
...
COINS = [
    ...
    "STX-USD","JTO-USD","PYTH-USD","GRT-USD","SEI-USD",
    "ENS-USD","FLOW-USD","KSM-USD","KAVA-USD",
    "WLD-USD","HBAR-USD","STRK-USD",
...
# ===== Simple in-memory cache for daily candles =====
_DAILY_CACHE = {}  # coin -> (df_1d, fetched_at_utc)
_DAILY_CACHE_TTL_SECS = 6*3600

def get_daily_df(coin: str):
    import time as _t
    now = _t.time()
    if coin in _DAILY_CACHE:
        df, ts = _DAILY_CACHE[coin]
        if now - ts < _DAILY_CACHE_TTL_SECS:
            return df
    df = fetch_with_warmup(coin, 86400, start_hours_ago=24*400)
    _DAILY_CACHE[coin] = (df, now)
    return df
...
def fetch_candles_range(product_id: str, granularity: int, start_dt: pd.Timestamp, end_dt: pd.Timestamp, session=None):
    ...
    while curr_end > start_dt:
        ...
        try:
            throttle(); r = get_session().get(url, params=params, timeout=20)
...
def fetch_ticker_price(product_id: str) -> float:
    ...
    throttle(); r = get_session().get(url, timeout=10)
    r.raise_for_status()
    data = r.json()
...
def tg_send(text: str):
    try:
        ...
        throttle(); r = get_session().post(url, json=payload, timeout=15)
        return r.ok
...
def process_coin(coin: str):
    ...
    # fetch data
    df_1h = fetch_with_warmup(coin, 3600, start_hours_ago=24*10)  # 10 days of 1H (lighter)
    df_1d = get_daily_df(coin)  # cached ~400 days of 1D
