...
API_KEY = "55c7094c-40c9-4f9a-846c-5ff1872c943d"
PRIVATE_KEY = """
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEICE2KwDptrnr70JslbJl8kuK9ukHTEP8F5uu1g4vethZoAoGCCqGSM49
AwEHoUQDQgAEFe7GYhcgj8O+tDq/MWkZ3FfiCl+aINOUgId65vk2aOANUk5YMZgT
uMkbVeSvPNzF1dM+N3/z98dLpddWufzLIA==
-----END EC PRIVATE KEY-----
"""

API_PASS = "your_passphrase"  # Add your passphrase here
...
def generate_cb_signature(request_path, method, body=""):
    timestamp = str(int(time.time()))
    message = f"{timestamp}{method}{request_path}{body}"
    message = message.encode("utf-8")
    secret = base64.b64decode(PRIVATE_KEY.encode("utf-8"))
...
def analyze_whale_orders(symbol, mid_price):
    whale_threshold = 1.0  # Dynamic threshold logic can be applied here
    request_path = f"/products/{symbol}/book?level=2"
    headers = generate_cb_signature(request_path, "GET")
...
def analyze_order_book(symbol, data):
    try:
        ...
        if long_signal:
            ...
            return (f"{symbol.upper()}: LONG Signal | Entry: {format_price(close_price)}, TP: {format_price(tp)}, SL: {format_price(sl)}\n"
            ...
            return (f"{symbol.upper()}: SHORT Signal | Entry: {format_price(close_price)}, TP: {format_price(tp)}, SL: {format_price(sl)}\n"
...
def fetch_historical_data(symbol):
    request_path = f"/products/{symbol}/candles?granularity=3600"
    headers = generate_cb_signature(request_path, "GET")
    url = f"{API_URL}{request_path}"
    response = requests.get(url, headers=headers)
    if response.status_code == 200:
