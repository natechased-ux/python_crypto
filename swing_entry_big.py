...
def get_ema_guidance_all(row, price):
    emas_above = [(p, row[f"ema{p}"]) for p in [10, 50, 100, 200] if row[f"ema{p}"] > price]
    emas_below = [(p, row[f"ema{p}"]) for p in [10, 50, 100, 200] if row[f"ema{p}"] < price]
...
def monitor_stoch_confirmation(symbol, side, entry, tp, sl, ema_msg, signal_time):
    ...
    while datetime.utcnow() < timeout:
        ...
        print(f"🕒 {symbol.upper()} dpre {prev_d:.4f}")
        
        if side == "long" and k > d and k < 35 and k > prev_k and d > prev_d:
            send_telegram_alert( f"✅big *LONG Confirmed* on {symbol.upper()}\nEntry: {entry:.4f}\nTP: {tp:.4f}\nSL: {sl:.4f}\n{signal_time}{ema_msg}")
            if symbol in open_confirmations:
                del open_confirmations[symbol]
            break

        elif side == "short" and k < d and k > 65 and k < prev_k and d < prev_d:
            send_telegram_alert(f"✅big *SHORT Confirmed* on {symbol.upper()}\nEntry: {entry:.4f}\nTP: {tp:.4f}\nSL: {sl:.4f}\n{signal_time}{ema_msg}")
            if symbol in open_confirmations:
                del open_confirmations[symbol]
            break
